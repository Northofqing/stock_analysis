...
### 2.1 全天补报（2026-09-23 收盘后回填，**这是本文最强的一条证据**）

上表截至 10:46。**收盘后回填全交易日（09:30:31 – 14:59:38）**：

| 项 | 全天值 |
| --- | --- |
| **provider=Tdx `outcome=available`** | **0** |
| provider=Tdx `outcome=unavailable` | **809** |
| provider=Baidu `outcome=available` | 1758（全程正常） |
| 被跳过的卖出评估 | **615** 次 |
| 受影响标的（去重） | **32 只**（10:46 时 19 只 → 收盘 32 只，**仍在扩大**） |

**请特别留意第一行：809 次调用、0 次成功、连续 8 小时无一次恢复。**

这排除了「间歇性抖动」「特定时段不可用」「开盘瞬时拥塞」等一切偶然性解释 ——
该路由是**确定性、持续性失效**，且失败标的集合**随时间扩大而非收敛**。
结合 §1 探针的 8/8 形状全败，我方判断这是**单点缺陷，不是负载或重试问题**。
