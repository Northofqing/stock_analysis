...
authorization/event state machine plus v5 companion active and immutable
attempt bindings. BR-192 and BR-194 land through one coherent v4-to-v5
migration and one shared v5 manifest; no sequential or competing v4-to-v5
...
must be conformed to these tasks. Contract Gate A passed against design blob
...
is closed by the exact cached-row proof recorded below. Gate B may proceed
against those identities; Gate B/C/D and live evidence remain pending.
...
| 2.3 Bad-data validation | N/A to this delivery-only slice | No price/series computation is added. The frozen upstream envelope remains authoritative and is not reinterpreted. |
| 2.4 Freshness | N/A to acquisition in this slice | Retry performs zero provider calls and preserves original source/observation evidence; it does not refresh or relabel stale data. |
...
here: current Shanghai business date, canonical A-share filter, `limit=20`,
one `VolumeRatio` and one `MainNetInflow` request through zero-argument
`EastmoneyProviderTopNRankingRouter::new()`, preserving provider order, row
date, declared total, inspected count, batch identity and observed time. It is
a single-response Provider TopN, never a complete-market ranking. Retry does
not call the provider, rerank, merge, fill or rerender.
...
authorities, and production CLI surfaces expose no path/test/capability
selector.
...
- Durable schema v4 is the repository baseline and already owns every BR-194
  terminal-replay object, audit kind and manifest semantic. Task 2 owns one
  shared additive v4-to-v5 migration containing the BR-192 and BR-194
  companion authorities; it must preserve every historical upgrade chain into
  the current v4 baseline and every v4 row/object unchanged. Fresh v0 plus
  v1/v2/v3/v4 upgrades must converge to the same complete v5 manifest.
...
- The existing sink seam is
  ...
  ```text
  $ rg -n -U 'pub type AuthoritativeSink\s*=|trait AuthoritativeSinkPort[\s\S]{0,800}fn deliver' src/durable_delivery
  src/durable_delivery/model.rs:1086:pub trait AuthoritativeSinkPort: Send + Sync {
  src/durable_delivery/model.rs:1087:    fn sink_identity(&self) -> &str;
  src/durable_delivery/model.rs:1088:    fn deliver(&self, request: &AuthoritativeDeliveryRequest) -> AuthoritativeSinkResult;
  src/durable_delivery/model.rs:1091:pub type AuthoritativeSink = Arc<dyn AuthoritativeSinkPort>;
  ```
...
- `RuntimeState` currently has five struct literals: the runtime constructor
  ...
  taken from the current source, not an earlier plan snapshot.

  ```text
  $ rg -n 'RuntimeState\s*\{' src/bin/monitor/durable_delivery_runtime.rs
  66:struct RuntimeState {
  1085:    Ok(Arc::new(RuntimeState {
  1695:        let state = Arc::new(RuntimeState {
  3632:        let state = RuntimeState {
  3694:        let state = RuntimeState {
  3732:        let restarted = RuntimeState {
  ```
...
- `ReconcileSummary` remains its existing eight-field type. Its single
  ...
  ```text
  $ sed -n '1161,1170p' src/durable_delivery/model.rs
  ...
  $ rg -n 'ReconcileSummary\s*\{|fn reconcile_startup_blocking|fn reconcile_current_decision|summary\.progress_count' src/durable_delivery/coordinator.rs src/bin/monitor/durable_delivery_runtime.rs
  src/bin/monitor/durable_delivery_runtime.rs:1120:fn reconcile_startup_blocking(state: &RuntimeState) -> Result<StartupReconcileEvidence, String> {
  src/bin/monitor/durable_delivery_runtime.rs:1133:        progress_count += summary.progress_count;
  src/bin/monitor/durable_delivery_runtime.rs:1218:fn reconcile_current_decision(
  src/bin/monitor/durable_delivery_runtime.rs:1229:        if summary.progress_count == 0 {
  src/durable_delivery/coordinator.rs:5470:            Ok(ReconcileSummary {
  ```
...
- `src/durable_delivery/mod.rs` currently declares only `coordinator`, `model`
  ...
  ```text
  $ rg -n '^mod (coordinator|model|schema);$' src/durable_delivery/mod.rs
  8:mod coordinator;
  9:mod model;
  10:mod schema;
  ```

- Existing schema-migration coverage must be renamed/updated to v5 while
  retaining fresh/v1/v2/v3/v4 input coverage, the existing v1/v2
  invalid-history regressions and byte-compatible preservation of the v4
  BR-194 manifest and rows.

  ```text
  $ rg -n '^fn br192_schema_v3_migration_matrix_is_repeatable_and_rejects_newer_versions' src/durable_delivery/tests.rs
  1835:fn br192_schema_v3_migration_matrix_is_repeatable_and_rejects_newer_versions() {
  ```
...
- The current real operator-auth boundary has a public
  ...
  ```text
  $ rg -n 'pub struct OperatorAuthConfig|pub fn load_auth_config|pub fn require_monitor_operator_auth|fn try_pam_auth|pam::Authenticator::with_password|auth\.authenticate\(\)' src/auth/operator.rs
  27:pub struct OperatorAuthConfig {
  52:pub fn load_auth_config() -> OperatorAuthConfig {
...
- After Task 8, rerun each command above. Expected deltas are: all five
  ...
  private modules and the one complete root export manifest exist; and the v5
  migration test retains fresh/v1/v2/v3/v4 fixtures plus BR-194 preservation.
...
- Modify `src/durable_delivery/model.rs`
  - owns frozen retry DTOs, namespace preimage/hash helper and serde validators
- Modify `src/durable_delivery/schema.rs`
  - owns central deterministic `sha256_hex` UDF registration/self-test,
    the one shared v4-to-v5 manifest/migration and atomic retry
...
- Modify `Cargo.toml`
  - add `rusqlite` feature `functions` while retaining `chrono`
...
- Create `tools/compliance/lib/check_br192_provider_free_retry.sh`
  - validates the complete shared v5 manifest and recomputes canonical hashes;
    logging-only/schema-name-only success is forbidden
- Modify `tools/compliance/lib/check_br194_review_dependency.sh`
  - recognizes the same v5 manifest without weakening any BR-194 invariant
...
No provider, Gateway, market-data or renderer production module is modified.

## Gate A documentation tracking

Gate A tracking result:
...
This metadata update changes no design, plan or business-rule contract. The
design and plan remain in the same PR as the implementation. Task 1 may begin
only against the accepted identities above. Gate B/C/D and live evidence
remain pending.

## Task order and dependency graph

```text
Task 1 BR registration/types
  -> Task 2 schema
        ...
        -> Task 5 unit and cross-process tests
          -> Task 6 authenticated command library types
            -> Task 7 read-only evidence library types
              -> Task 8 one atomic root/runtime/CLI/static/isolation integration
...
Register the single shared schema-v5 transition-event/hash authority and
...
Expected: compile/test failure because the new types/constants do not exist.
Verify output contains `running 1 test`; zero-test output is failure.

- [ ] **Step 4: Add the exact types**

Add the design enums and:
...
// `Err` from begin is commit-ambiguous and carries no release proof.
...
Do not modify `src/durable_delivery/mod.rs` in Task 1. Freeze this exact
...
Expected: all six run one test and pass.

```bash
git add docs/superpowers/specs/2026-07-30-br192-provider-free-retry-design.md docs/superpowers/plans/2026-07-30-br192-provider-free-retry.md docs/business_rules.md src/durable_delivery/model.rs src/durable_delivery/tests.rs
git commit -m "design: freeze BR-192 retry contract"
```

### Task 2: Add the one shared v5 manifest, deterministic SHA-256 authority,
...
```rust
#[test]
fn br192_retry_authorization_payload_is_immutable_and_unique_per_rejection() {}

#[test]
fn br192_retry_authorization_state_transitions_are_monotonic() {}

#[test]
fn br192_retry_authorization_transition_events_append_before_apply_or_invalidate() {}

#[test]
fn br192_authorization_append_acknowledgements_store_no_untrusted_timestamp() {}

#[test]
fn br192_active_binding_is_unique_and_historical_binding_survives_current_change() {}

#[test]
fn br192_attempt_binding_freezes_authorization_disposition_cycle_generations_owner_and_fence() {}

#[test]
fn br192_retry_reserved_attempt_binding_and_schedule_relation_is_atomic() {}

#[test]
fn br192_retry_fence_is_positive_integer_i64_across_schema_and_contract() {}

#[test]
fn br192_retry_send_ownership_is_retained_consumed_and_monotonic() {}

#[test]
fn br192_retry_cycle_payload_is_append_only_and_retained() {}

#[test]
fn br192_retry_cycle_ordinal_is_positive_unique_immutable_and_non_deletable() {}

#[test]
fn br192_retry_cycle_identity_rederives_from_retained_ordinal_and_exact_fields() {}

#[test]
fn br192_retry_schedule_is_retained_and_all_authority_fields_are_monotonic() {}

#[test]
fn br192_retry_schedule_last_attempt_is_fk_bound_to_exact_retry_attempt_ordinal() {}

#[test]
fn br192_frozen_rejection_atomically_initializes_schedule_from_observed_at() {}

#[test]
fn br192_retry_cycle_logical_slots_are_unique_and_conflicting_bytes_fail_closed() {}

#[test]
fn br192_retry_cycle_global_started_and_terminal_cardinality_is_exact() {}

#[test]
fn br192_terminal_phase_forbids_opposite_terminal_kind_after_pending_slot() {}

#[test]
fn br192_completed_append_error_leaves_running_completion_pending() {}

#[test]
fn br192_completed_ack_before_terminal_cas_leaves_running_completion_appended() {}

#[test]
fn br192_completed_terminal_cas_requires_exact_appended_completion_bytes() {}

#[test]
fn br192_failed_cycle_requires_closed_reason_and_exact_typed_field_digest() {}

#[test]
fn br192_retry_cycle_failure_is_opaque_non_deserializable_and_digest_is_recomputed() {}

#[test]
fn br192_failure_payload_persists_complete_canonical_preimage_envelope_and_hashes() {}

#[test]
fn br192_prior_boot_terminalizer_uses_only_persisted_failure_payload_bytes() {}

#[test]
fn br192_failed_append_error_leaves_running_failure_pending() {}

#[test]
fn br192_failed_ack_before_terminal_cas_leaves_running_failure_appended() {}

#[test]
fn br192_failed_terminal_cas_requires_exact_appended_failure_bytes() {}

#[test]
fn br192_sink_calls_are_not_started_confirmed_or_indeterminate_never_default_zero() {}

#[test]
fn br192_schema_v5_fresh_and_v1_v2_v3_v4_upgrade_paths_validate() {}

#[test]
fn br192_schema_v5_cycle_ordinal_manifest_is_identical_across_v0_v1_v2_v3_v4() {}

#[test]
fn br192_schema_v5_repeated_initialization_is_idempotent() {}

#[test]
fn br192_schema_newer_than_v5_fails_before_mutation() {}

#[test]
fn br192_all_v5_immutable_refs_reject_empty_and_ascii_whitespace_only() {}

#[test]
fn br192_v4_to_v5_preserves_br194_replay_manifest_audit_kinds_and_rows() {}

#[test]
fn br192_durable_sha256_udf_is_registered_before_every_schema_path() {}

#[test]
fn br192_durable_sha256_udf_registration_follows_complete_descriptor_binding() {}

#[test]
fn br192_durable_sha256_udf_never_runs_before_wal_shm_attestation() {}

#[test]
fn br192_rusqlite_031_exposes_utf8_deterministic_and_innocuous_function_flags() {}

#[test]
fn br192_durable_sha256_udf_rejects_null_text_and_wrong_type() {}

#[test]
fn br192_v5_authority_triggers_recompute_canonical_sha256() {}

#[test]
fn br192_v5_authority_triggers_reject_bytes_hash_and_combined_mutations() {}

#[test]
fn br192_python_br194_verifier_uses_hashlib_without_sql_callback_or_trigger_execution() {}
```
...
cargo test --lib durable_delivery::tests::br192_schema_v5_fresh_and_v1_v2_v3_v4_upgrade_paths_validate -- --exact --test-threads=1
cargo test --lib durable_delivery::tests::br192_schema_v5_cycle_ordinal_manifest_is_identical_across_v0_v1_v2_v3_v4 -- --exact --test-threads=1
cargo test --lib durable_delivery::tests::br192_schema_v5_repeated_initialization_is_idempotent -- --exact --test-threads=1
cargo test --lib durable_delivery::tests::br192_schema_newer_than_v5_fails_before_mutation -- --exact --test-threads=1
cargo test --lib durable_delivery::tests::br192_all_v5_immutable_refs_reject_empty_and_ascii_whitespace_only -- --exact --test-threads=1
cargo test --lib durable_delivery::tests::br192_v4_to_v5_preserves_br194_replay_manifest_audit_kinds_and_rows -- --exact --test-threads=1
...
cargo test --lib durable_delivery::tests::br192_v5_authority_triggers_recompute_canonical_sha256 -- --exact --test-threads=1
cargo test --lib durable_delivery::tests::br192_v5_authority_triggers_reject_bytes_hash_and_combined_mutations -- --exact --test-threads=1
...
Schema v4 is the current repository baseline and already owns BR-194 replay
authority. Add exactly one shared `migrate_durable_delivery_v4_to_v5`
transaction for
the BR-192 and BR-194 companion objects. Do not add a second feature-specific
v4-to-v5 step. Preserve ordered fall-through:

```text
user_version=0  -> create complete v5 directly
user_version=1  -> v1 -> v2 -> v3 -> v4 -> v5
user_version=2  -> v2 -> v3 -> v4 -> v5
user_version=3  -> v3 -> v4 -> v5
user_version=4  -> one shared v4 -> v5 transaction
user_version=5  -> validate only; repeat safely
user_version>5  -> fail before mutation
```

Each migration and its post-validation remain inside one transaction with
foreign-key enforcement enabled. Set `user_version=5` only after all required
v4 baseline and v5 companion tables, columns, foreign keys, indexes and
...
No UDF, configuration, schema read, DDL or transaction may precede complete
descriptor binding; no schema work or application query may precede UDF
registration/self-test. Centralize that post-binding/pre-configuration step so
no Rust caller bypasses it. The shared checker must prove this exact order.

Every v5 BR-192 and BR-194 authority trigger that accepts canonical bytes plus
a stored digest must require
`sha256_hex(canonical_blob)=stored_lowercase_sha256`. Migration replaces only
the weaker v4 BR-194 insert triggers with equivalent v5 definitions containing
this recomputation; it preserves every other v4 object and row. The Rust
...
CREATE TABLE retry_send_ownership(
  ...
  ownership_state TEXT NOT NULL
    CHECK(ownership_state IN (
      'Started','TerminalRecorded','InterruptedUncertain'
    )),
  ...
  CHECK(
    ...
    OR
    (ownership_state IN ('TerminalRecorded','InterruptedUncertain')
...
interrupted cycle to `Confirmed/0`.
...
`retry_authorization_bindings` is the v5 current reference. Fresh v0 creation
and every v1/v2/v3/v4-to-v5 upgrade create exactly this companion-table
manifest. Every fresh or migrated v5 `retry_cycles` row therefore includes the
same positive unique `cycle_ordinal`; the v5 manifest includes its column,
...
The shared v4-to-v5 transaction snapshots every existing decision identity,
row count and canonical/hash column plus every BR-194 replay object, audit
kind and manifest definition before creating objects. It copies no v4
...
before `user_version=5` is the transaction's final statement. Any mismatch
rolls back all objects and the version. Tests compare a fresh-v5 manifest to
each upgraded manifest and prove existing v4 data and all BR-194 replay
...
- `retry_authorizations` canonical/source/identity/`authorized_at` fields
  cannot update and no row can be deleted; manual `authorized_at` is copied
  exactly from the validated PAM session `validated_at`, while `created_at`
  remains insertion metadata and cannot affect eligibility;
...
- `retry_send_ownership` rejects update/delete of identity, binding,
  ...
  `Started -> TerminalRecorded|InterruptedUncertain` is legal after ordinary
  claim, plus the atomic appended-start quarantine may directly insert
  `InterruptedUncertain` to consume an otherwise unowned send right;
- `TerminalRecorded` requires the exact authoritative terminal sink-result row
  written by `record_sink_result` in the same transaction;
...
- `retry_schedules` rejects every deletion, a lower attempt count, a version
  ...
  `automatic_attempts_started`. Creating a FrozenRejection authorization and
  its rejection disposition must insert the zero-attempt schedule in the same
  transaction with `next_eligible_at=observed_at + 30s`;
...
candidate schedule lookup.

SQLite migrations must use the repository's idempotent column/table detection;
there is no v5 `ALTER TABLE delivery_decisions`. Detection does not replace
schema versioning: a database claiming v5 while any required companion object
...
one character remains. Do not add a divergent v5 validator. Every v5 append
...
- any fresh/upgraded v5 manifest that omits or changes a preserved v4 BR-194
  replay object, audit kind, trigger semantic or row;
- any authority row/outbox whose stored lowercase digest differs from
  `sha256_hex(canonical_blob)`, and any v5 trigger catalog that omits the same
  recomputation;
...
- a FrozenRejection authorization/disposition without the same-transaction
  zero-attempt schedule at exact `observed_at + 30s`;
...
- `TerminalRecorded` ownership without its exact authoritative terminal result,
  or `InterruptedUncertain` ownership with any terminal result or any reason
  other than `ProcessInterruptedAfterSinkStart`;
...
immutability or retained-row delete protection; complete v5 manifest
post-validation must reject every fixture before advancing `user_version`.

Rename the existing
`br192_schema_v3_migration_matrix_is_repeatable_and_rejects_newer_versions`
test exactly to
`br192_schema_v5_migration_matrix_is_repeatable_and_rejects_newer_versions`
and update all assertion text to fresh/migrated/reopened v5. Preserve its
existing v1/v2 invalid-history cases. The migration tests build isolated
databases at v0, v1, v2, v3 and v4, initialize them to v5, compare the complete
required-object manifest, initialize v5 again, prove v4
...
and no append-authority timestamp column, and verify `user_version=6` is
rejected without before/after metadata or schema change.
...
cargo test --lib durable_delivery::tests::br192_retry_cycle_global_started_and_terminal_cardinality_is_exact -- --exact --test-threads=1
cargo test --lib durable_delivery::tests::br192_schema_v5_cycle_ordinal_manifest_is_identical_across_v0_v1_v2_v3_v4 -- --exact --test-threads=1
cargo test --lib durable_delivery::tests::br192_schema_v5_ -- --test-threads=1
cargo test --lib durable_delivery::tests::br192_schema_newer_than_v5_fails_before_mutation -- --exact --test-threads=1
cargo test --lib durable_delivery::tests::br192_all_v5_immutable_refs_reject_empty_and_ascii_whitespace_only -- --exact --test-threads=1
cargo test --lib durable_delivery::tests::br192_v4_to_v5_preserves_br194_replay_manifest_audit_kinds_and_rows -- --exact --test-threads=1
cargo test --lib durable_delivery::tests::br192_durable_sha256_udf_is_registered_before_every_schema_path -- --exact --test-threads=1
...
git commit -m "feat: add shared BR-192 BR-194 v5 authority schema"
```
...
```rust
#[test]
fn br192_envelope_retry_boolean_never_authorizes_without_current_frozen_authority() {}

#[test]
fn br192_frozen_rejection_authority_requires_appended_disposition_and_authorization() {}

#[test]
fn br192_frozen_rejection_authorization_and_initial_schedule_share_one_transaction() {}

#[test]
fn br192_authorization_recovers_insert_append_ack_and_apply_crashes() {}

#[test]
fn br192_authorization_apply_and_invalidate_events_recover_before_projection_cas() {}

#[test]
fn br192_identical_authorization_replay_returns_stored_record() {}

#[test]
fn br192_different_authorization_bytes_for_same_rejection_fail_closed() {}

#[test]
fn br192_stale_authorization_is_invalidated_not_retargeted() {}

#[test]
fn br192_pending_authorization_blocks_cycle_before_candidate_query_and_sink() {}

#[test]
fn br192_new_disposition_clears_active_binding_but_preserves_history() {}

#[test]
fn br192_authorization_reconcile_bound_returns_exact_typed_variant() {}
```
...
```rust
#[test]
fn br192_every_retry_deferral_has_one_cycle_bound_immutable_audit() {}

#[test]
fn br192_every_retry_ineligibility_has_one_cycle_bound_immutable_audit() {}

#[test]
fn br192_missing_decision_returns_typed_error_before_admission_result_or_audit() {}

#[test]
fn br192_candidate_query_never_returns_a_missing_decision_identity() {}

#[test]
fn br192_reserved_selector_returns_all_257_rows_in_total_order_without_truncation() {}

#[test]
fn br192_prior_boot_selector_returns_all_257_rows_before_new_cycle() {}

#[test]
fn br192_retry_candidate_selector_returns_all_257_rows_in_one_frozen_snapshot() {}

#[test]
fn br192_primary_selector_source_rejects_limit_offset_cursor_and_caller_cardinality() {}

#[test]
fn br192_admission_revalidates_envelope_hash_and_all_frozen_policy_fields() {}

#[test]
fn br192_backoff_is_persisted_and_attempt_cap_exhausts_once() {}

#[test]
fn br192_uncertain_states_never_enter_candidates_or_sink_path() {}

#[test]
fn br192_entering_uncertain_manual_review_clears_active_binding_and_preserves_history() {}

#[test]
fn br192_admission_atomically_installs_attempt_binding_schedule_and_reserved() {}

#[test]
fn br192_prepare_uses_existing_binding_and_never_creates_generation_or_attempt() {}

#[test]
fn br192_pending_sink_attempt_started_event_never_calls_sink() {}

#[test]
fn br192_unavailable_sink_attempt_started_returns_typed_error_and_failed_cycle_audit() {}

#[test]
fn br192_start_audit_unavailable_prestart_reason_matrix_preserves_rows_hashes_and_zero_sink() {}

#[test]
fn br192_start_audit_unavailable_post_ack_or_consumed_rejects_exception_and_quarantines() {}

#[test]
fn br192_appended_sink_attempt_started_validator_rejects_each_identity_hash_ref_generation_and_fence_mismatch() {}

#[test]
fn br192_sink_attempt_started_time_equals_prepared_canonical_and_persisted_outbox_time() {}

#[test]
fn br192_crash_after_admission_before_prepare_recovers_same_binding_without_new_generation_or_sink_duplication() {}

#[test]
fn br192_crash_after_pre_call_cas_before_sink_is_quarantined_without_resend() {}

#[test]
fn br192_remote_accept_then_crash_before_result_is_quarantined_without_resend() {}

#[test]
fn br192_same_process_repeated_execute_consumes_one_permit_and_calls_sink_once() {}

#[test]
fn br192_record_sink_result_still_requires_attempt_in_flight() {}

#[test]
fn br192_persisted_retry_sink_outcome_joins_terminal_result_and_ownership() {}

#[test]
fn br192_result_insert_then_ownership_update_failure_rolls_back_and_quarantines_without_resend() {}

#[test]
fn br192_cycle_begin_rejects_empty_and_ascii_whitespace_boot_identity_before_insert() {}

#[test]
fn br192_cycle_begin_persists_exact_explicit_boot_identity() {}

#[test]
fn br192_cycle_begin_derives_identity_before_running_check_and_binds_no_commit_proof() {}

#[test]
fn br192_no_commit_branch_queries_zero_proposed_cycle_and_started_before_rollback() {}

#[test]
fn br192_consume_no_commit_proof_rederives_next_identity_and_rejects_concurrent_change() {}

#[test]
fn br192_no_commit_proof_rejects_domain_schema_field_order_encoding_and_witness_mutations() {}

#[test]
fn br192_empty_running_check_atomically_persists_exact_ordinal_identity_and_started() {}

#[test]
fn br192_cycle_ordinal_exhaustion_returns_exact_typed_variant_without_write() {}

#[test]
fn br192_cycle_audit_reconcile_bound_returns_exact_typed_variant() {}
```
...
cargo test --lib durable_delivery::tests::br192_result_insert_then_ownership_update_failure_rolls_back_and_quarantines_without_resend -- --exact --test-threads=1
...
advances ownership to `TerminalRecorded`, proves the cycle has no remaining
nonterminal consumed ownership, counts exact joined results and changes
...
and schedule FK, validates the provided execution cycle is `Running`, and
idempotently inserts that execution cycle's exact pending
`SinkAttemptStarted` logical-slot outbox row. It returns only the token for
that already-existing attempt, carrying both execution and admission cycle
...
capability. In one `BEGIN IMMEDIATE` it reruns the validator/read-back, CASes
the exact current attempt from `Reserved -> AttemptInFlight`, and inserts
...
winner receives the non-clone, non-serializable permit.
`DurableDeliveryCoordinator::execute_prepared_retry_sink` is the only seam
with a sink capability, consumes that permit by value, revalidates
`AttemptInFlight` plus the exact `Started` ownership, and performs one external
call. Before returning, it immediately invokes `self.record_sink_result` on
the same coordinator. That transaction continues to require
`decision=AttemptInFlight`, inserts the authoritative result and advances
ownership to `TerminalRecorded` atomically. The method returns only
`PersistedRetrySinkOutcome` with the stored result identity and persisted
decision/ownership states; it never returns a bare `AuthoritativeSinkResult`.

Inject a transaction failure after authoritative result insert but before the
ownership update. The insert, state transition and ownership transition must
all roll back, leaving no result row, decision `AttemptInFlight` and ownership
`Started`. Startup/cycle recovery then conservatively records
...
`PersistedRetrySinkOutcome` that joins the result row, decision terminal state
...
coordinator.execute_prepared_retry_sink
```
...
```rust
#[test]
fn br192_reserved_phase_requeries_new_rejection_and_duplicate_suppresses_second_sink() {}

#[test]
fn br192_test_control_without_attempted_set_still_cannot_bypass_send_claim() {}

#[test]
fn br192_startup_reserved_reentry_reuses_admission_binding_without_new_generation_or_second_sink() {}

#[test]
fn br192_startup_quarantines_prior_boot_started_or_inflight_without_terminal_result() {}
```
...
```rust
#[test]
fn br192_two_processes_race_one_retry_with_one_generation_and_one_sink_call() {}

#[test]
fn br192_two_processes_race_pre_call_cas_with_one_consumed_owner_and_one_sink_call() {}

#[test]
#[ignore = "invoked only by the BR-192 parent process harness"]
fn br192_retry_cross_process_child() {}
```
...
```rust
#[tokio::test]
async fn br192_retry_cycle_measures_zero_provider_and_renderer_calls() {}

#[tokio::test]
async fn br192_cycle_guard_survives_async_cancellation_until_blocking_exit() {}

#[tokio::test]
async fn br192_cycle_guard_covers_sink_and_completed_or_failed_terminalization() {}

#[tokio::test]
async fn br192_cycle_guard_latches_running_when_failure_pending_is_not_durable() {}

#[tokio::test]
async fn br192_precycle_namespace_error_never_acquires_guard() {}

#[tokio::test]
async fn br192_begin_not_committed_proof_releases_guard() {}

#[tokio::test]
async fn br192_no_commit_proof_concurrent_change_latches_outer_guard() {}

#[tokio::test]
async fn br192_begin_commit_ambiguous_error_latches_guard() {}

#[test]
fn br192_retry_runtime_boundary_uses_only_durable_result_and_exact_failure_constructors() {}

#[tokio::test]
async fn br192_begin_not_committed_propagates_exact_typed_error_after_proof_release() {}

#[tokio::test]
async fn br192_begin_commit_ambiguous_propagates_exact_typed_error_and_latches_guard() {}

#[test]
fn br192_guard_failures_return_exact_typed_variants_without_string_downgrade() {}

#[tokio::test]
async fn br192_normal_error_after_claim_quarantines_same_cycle_attempt_before_failed() {}

#[tokio::test]
async fn br192_caught_panic_after_claim_quarantines_same_cycle_attempt_before_failed() {}

#[tokio::test]
async fn br192_completion_pending_error_resumes_completed_without_failed_slot() {}

#[tokio::test(start_paused = true)]
async fn br192_periodic_runner_delays_first_tick_thirty_seconds() {}

#[tokio::test]
async fn br192_startup_and_periodic_paths_share_one_cycle_algorithm() {}

#[tokio::test]
async fn br192_cycle_identity_is_durable_before_sink_capable_spawn() {}

#[tokio::test]
async fn br192_boot_authority_rejects_missing_empty_and_ascii_whitespace_before_cycle_insert() {}

#[tokio::test]
async fn br192_cycle_persists_exact_preopen_boot_authority_identity() {}

#[tokio::test]
async fn br192_blocking_panic_is_caught_and_failed_event_is_appended() {}

#[tokio::test]
async fn br192_runtime_preserves_typed_start_audit_failure_through_failed_join() {}

#[tokio::test]
async fn br192_join_error_recovers_retained_cycle_identity() {}

#[tokio::test]
async fn br192_join_error_quarantines_started_or_inflight_attempt_before_cycle_failure() {}

#[tokio::test]
async fn br192_join_orphan_and_cancellation_recovery_capabilities_exclude_sink_provider_renderer() {}

#[tokio::test]
async fn br192_same_boot_running_cycle_is_not_orphan_recovered() {}

#[tokio::test]
async fn br192_next_startup_recovers_prior_boot_running_cycle() {}

#[tokio::test]
async fn br192_prior_boot_recovery_append_acks_uncertainty_before_failure_prepare() {}

#[tokio::test]
async fn br192_completion_pending_reboot_resumes_completed_without_failed_slot() {}

#[tokio::test]
async fn br192_completion_appended_reboot_resumes_completed_without_failed_slot() {}

#[tokio::test]
async fn br192_failure_pending_reboot_uses_only_persisted_bytes() {}

#[tokio::test]
async fn br192_failure_appended_reboot_uses_only_persisted_bytes() {}

#[tokio::test]
async fn br192_failure_reboot_corruption_matrix_fails_closed() {}

#[test]
#[ignore = "invoked only by the BR-192 boot-recovery parent harness"]
fn br192_boot_recovery_process_child() {}

#[test]
#[ignore = "invoked only by the BR-192 terminal-reboot parent harness"]
fn br192_terminal_reboot_process_child() {}
```
...
Process Reserved identities first. Reconcile to a fixed point, then query retry
candidates. The same set is passed to both phases. Insert before the sink
...
schedule and all three identities, `next_eligible_at<=now`, and
`exhausted_at IS NULL`. No rowid, caller/provider order, implicit NULL order,
...
Use the existing shutdown token/select. Emit the startup banner once. Publish
`delivery.retry.cycle` only after the matching durable cycle terminal event is
appended.
...
`PersistedRetrySinkOutcome`, reconciles cycle audit and uses one
...
5. apply/reconcile the authorization and return only
   `ProductionRetryAuthorizationOutcome`.
...
```rust
#[test]
fn br192_test_attestation_can_mutate_only_nonce_bound_test_target() {}

#[test]
fn br192_production_target_rejects_test_attestation_before_open() {}

#[test]
fn br192_manual_authorization_identical_replay_returns_stored_identity() {}

#[test]
fn br192_manual_authorization_rejects_all_three_uncertain_states() {}

#[test]
fn br192_production_authority_rejects_before_open_when_not_authenticated() {}

#[test]
fn br192_authenticated_retry_operator_validates_kind_timestamp_and_session_evidence() {}

#[test]
fn br192_resolver_is_unreachable_before_authentication_and_target_compatibility() {}

#[test]
fn br192_resolved_production_target_cannot_hold_test_authority_root_or_capability() {}

#[test]
fn br192_manual_target_namespace_is_derived_after_auth_before_open_and_persisted() {}

#[test]
fn br192_manual_namespace_tampering_and_production_test_mismatch_fail_before_open() {}

#[test]
fn br192_non_test_public_surface_has_no_injectable_authority_resolver_or_target() {}

#[test]
fn br192_production_entry_owns_pam_and_fixed_manifest_resolver() {}

#[test]
fn br192_authorization_time_equals_authority_validated_at_even_when_command_is_delayed() {}

#[test]
fn br192_authorization_request_has_no_caller_controlled_timestamp() {}

#[test]
fn br192_operator_auth_attestation_is_opaque_and_created_only_after_pam_success() {}

#[test]
fn br192_retry_command_consumes_attestation_without_constructing_time_subject_service_or_nonce() {}

#[test]
fn br192_require_monitor_operator_auth_delegates_and_discards_attestation() {}
```
...
```rust
#[test]
fn br192_retry_evidence_dtos_are_deny_unknown_and_redacted() {}

#[test]
fn br192_verified_retry_evidence_and_cli_share_exact_required_fields() {}

#[test]
fn br192_non_test_evidence_surface_exposes_only_production_verifier() {}

#[test]
fn br192_test_evidence_verifier_exact_join_is_read_only_and_nonce_bound() {}

#[test]
fn br192_evidence_verifier_accepts_and_deduplicates_byte_identical_artifact_replay() {}

#[test]
fn br192_evidence_verifier_rejects_zero_conflicting_duplicate_mismatch_and_write_capable_target() {}

#[test]
fn br192_evidence_query_count_bounds_apply_before_authority_open() {}

#[test]
fn br192_evidence_stream_rejects_257th_distinct_join_without_partial_output() {}

#[test]
fn br192_evidence_query_count_zero_returns_exact_typed_variant_before_authority_open() {}

#[test]
fn br192_evidence_query_count_257_returns_exact_typed_variant_before_authority_open() {}

#[test]
fn br192_evidence_logical_tuple_hash_matches_frozen_golden_and_recomputes() {}

#[test]
fn br192_evidence_logical_tuple_hash_rejects_domain_schema_field_order_and_encoding_mutations() {}

#[test]
fn br192_evidence_conflicting_canonical_bytes_returns_exact_typed_variant() {}

#[test]
fn br192_evidence_conflicting_canonical_hash_returns_exact_typed_variant() {}

#[test]
fn br192_evidence_conflicting_canonical_bytes_and_hash_returns_exact_typed_variant() {}

#[test]
fn br192_evidence_conflicting_duplicate_zero_flags_rejected_before_variant_construction() {}

#[test]
fn br192_evidence_257th_distinct_returns_exact_typed_variant_without_partial_output() {}
```
...
- successful execute returning `PersistedRetrySinkOutcome` whose stored result
  identity, terminal decision state and `TerminalRecorded` ownership state join
  exactly; inject a `record_sink_result` transaction failure after
  authoritative-result insert but before ownership update and assert the whole
  transaction rolls back, the decision remains `AttemptInFlight`, ownership
  remains `Started`, no result row remains, and recovery quarantines with zero
  resend;
...
Add this exact compile-contract test in this task, after all owning symbols
exist:
...
`br192_result_insert_then_ownership_update_failure_rolls_back_and_quarantines_without_resend`
tests against the integrated public graph; it does not redefine them.

Add these exact constructor contract tests after the root re-export exists:
...
`print_redacted_authorization_outcome` prints only the six exported outcome
fields and is defined in the binary; it performs no authority, resolution,
database, append or evidence operation. There is no alternate run function or
hidden target/root/time flag. Every imported authorization symbol appears in
the frozen root manifest.

The binary owns a checked-in structured unit test:

```rust
#[test]
fn br192_authorization_cli_request_surface_is_timestamp_and_capability_free() {}
```
...
`ProductionRetryAuthorizationOutcome.authorized_at` is a required legitimate
...
```rust
#[test]
fn br192_verify_evidence_cli_has_exact_production_arguments_and_library_call() {}

#[test]
fn br192_verify_evidence_cli_rejects_out_of_range_count_before_library_call() {}
```
...
```bash
git diff --unified=0 -- '*.rs' | rg 'unwrap_or_default\\(|let _ = .*\\.await|Err\\(.+\\) => \\{\\}|if .* \\{$'
```
...
```bash
git add Cargo.toml src/durable_delivery/mod.rs src/durable_delivery/tests.rs src/bin/monitor/durable_delivery_runtime.rs src/bin/monitor/main.rs src/bin/authorize_delivery_retry.rs src/bin/verify_br192_retry_evidence.rs tests/durable_delivery_counted_cutover.rs tests/monitor_help_isolation.rs
...
the same v5 manifest and deterministic canonical SHA-256 contract. Rust checks
...
Expected: global line coverage at least 80%, core trading/data paths at least
95%; checker exits 0. LCOV is not passed to the JSON checker.
...
Expected: at least one startup banner, at least one delayed cycle after 30
seconds, graceful shutdown, no overlapping-cycle error and no surviving
process launched by this harness. Do not use a broad `pkill`.

- [ ] **Step 8: Production push and audit evidence**

For a real `ReviewProviderTopN` counted retry, run only the checked-in,
read-only exact verifier:

```bash
DATE=$(date +%Y-%m-%d)
./target/release/verify_br192_retry_evidence \
  --date "${DATE}" \
  --push-kind ReviewProviderTopN \
  --require-count 1
```

Expected: exit 0 and one redacted JSON result with
`durable_push_kind="ReviewProviderTopN"`, `verified_retry_count>=1`,
`exact_join=true` and only hashes/counts. The parser reads the JSON
`durable_push_kind` field; rendered text and shell grep are not evidence. If no
real authorized rejection occurs in the observation window, the verifier exits
non-zero and Gate D is **Blocked: production retry evidence unavailable**; do
not create one or relax safety.

Use the checked-in BR-192 verifier/query to join one real retried terminal:
...
Expected: active `ReviewProviderTopN` has a real producer/counted caller.
Legacy annotations have an explicit deletion plan or are recorded as upstream
debt; no “zero caller” claim uses single-line grep.

- [ ] **Step 10: Independent review and PR evidence**

A fresh reviewer must start from:

> DO NOT trust the implementer's report as ground truth. Independently rerun
> tests/build, inspect multiline production call sites, verify push-log and
> event-audit evidence, and check cross-version debt.

The PR must include:

```text
Refs: design §1-15; BR-192
Data-Redlines: [2.1, 2.2, 2.3, 2.4, 2.5, 2.6, 2.7, 2.8, 2.9, 2.10]
Data-Redline-Evidence: plan “Data-red-line execution matrix”; 2.3/2.4/2.6/2.9 are scoped N/A proofs, all others require positive evidence
OldModules: design §13 table
Threshold-Proof: retry constants govern delivery recovery only; no financial threshold changed
Business-Rules: BR-192
Rollback: design §14
...
1. stop the monitor;
2. revert the retry runtime/CLI behavior in a rollback branch, but retain or
   immediately reapply Task 2's v5 schema recognition, deterministic
   `sha256_hex` registration and complete shared-manifest validation before
   any rollback binary is built;
3. retain `SCHEMA_VERSION=5` recognition/validation, all v4 baseline and v5
...
4. never launch a historical schema-v2/v3/v4 binary, lower `user_version`,
   unregister `sha256_hex` or remove v4 baseline/v5 companion objects;
5. do not restore automatic boolean `reacquire_rejected`;
6. build the new v5-aware rollback release binary from the previous runtime
   behavior;
7. restart with retry runner absent/disabled at its single startup call site;
8. verify previous reconciliation handles already-Reserved/pending terminal
   bytes only; and
