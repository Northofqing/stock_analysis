...
`core_file_count=218`、工具身份、PR core/other patch 下限 90%/85%、Release global/core
...
core 仅 `77.87%`，历史 Gate D 覆盖率债务继续 `Release Blocked`；production freshness
明确 `NOT RUN`，live provider、生产数据库、审计签字和部署证据均不在本任务范围。
