...
该调度器是 19:00 严格复盘的唯一生产 owner。必须删除 `monitor_loop` 内旧的 `evening_pushed` 状态和直接 dispatcher 调用；保留两条生产路径会同时造成死路径、极端并发重复和“全失败仍提交完成”的矛盾语义。
...
常驻 main
  ...
  -> BR-140 逐任务提交状态
     Delivered/NoData/Disabled：该任务当日终结
     ExpectedWait/可重试 Failed：仅该任务后续重试
```
...
- 至少一份确认投递：只终结已投递任务；其余等待或可重试子报告继续按 BR-140 调度，避免 19:00 成功吞掉 21:00 龙虎榜。
...
生产接线回归还必须断言 `main.rs` 只有严格 inner runner 一个 `dispatch_post_session_review` 调用点，并且旧 `evening_pushed` owner 不存在。
