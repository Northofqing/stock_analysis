...
事件或产业链关系命中只产生“待评估证券”，不等于正式候选。首期采用可复验、无评分的 `admission-v1` 硬门禁；证券必须同时满足：

- MA5、MA10、MA20 均存在且为有限正数，并且 `MA5 >= MA10 >= MA20`。
- 当前/最新结算价格不低于 MA5，且相对 MA20 的距离在 `[0, 15%]`。
- 5 日收益在 `[0, 20%]`，既要求正向动量，也拒绝未经样本校准的过度追高。
- 最新结算日成交量同时不低于前 5 日和前 20 日真实均量，即两个量能比都 `>= 1.0`。
- 交易时段还要求真实同时间槽量能节奏比存在且 `>= 1.0`；闭市不伪造该特征，也不以它作为闭市门禁。

任一必需特征缺失、非有限或不满足门禁时，证券产生确定性的结构化拒绝并从正式候选批次剔除。被剔除证券不得写入 `selection_candidates` / `selection_feature_snapshots`，不得获得 `selection_visibility_receipts`，不得进入 T0、D+1 outcome、回测样本或任何候选推送；拒绝事实只进入权威审计和事件完成摘要。门禁一次返回全部拒绝原因，不以单一首错掩盖其他问题。

上述数值是 shadow 阶段的 `admission-v1` 基线，不代表预测成功率。修改任何边界必须同步本节、BR-156、门禁测试和后续真实样本对比；在样本校准前不得自动放宽、个票例外或改成隐式分数。
...
仅通过 §6.4 `admission-v1` 硬门禁的正式影子候选写入 `selection_runs`、`selection_candidates` 和 `selection_feature_snapshots`。T0 快照包含当时事件、关系和市场证据的身份及原始特征，写入后不更新、不删除。
