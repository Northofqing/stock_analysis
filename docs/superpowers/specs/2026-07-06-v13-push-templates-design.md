...
## 0.1 版本命名约定（2026-07-06 用户决定）

按 semver 习惯，本 spec 实施用**子版本** (v13.x) 而非独立大版本。commit 历史已用 v14-v17 命名（不可改），从 v18+ 开始严格用 v13.x 子版本：

| 实际 commit 命名 | 应该是 | 内容 | 关联 spec |
|---|---|---|---|
| v14 | **v13.1** | A-01 收尾 + 文档修正 + T-08 拆分 | §3.5/§3.7/§6 |
| v15 | **v13.2** | 6 wrapper + 6 业务集成抽口 | §3 |
| v16 | **v13.3** | 5 真实数据源集成 | §3 |
| v17 | **v13.4** | main.rs 调度 + 关键词 + 实时 + JSON 解析 | §3 |
| v18+ | **v13.5+** | 后续 v13 spec 实施 | 持续 |

**新约定**：v13 spec 范围内的工作用 v13.x 子版本；新大版本（v14/v15）只在**新 spec** 启动时使用。
