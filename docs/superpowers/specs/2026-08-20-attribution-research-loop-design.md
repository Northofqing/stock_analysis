...
结构性重建失败目前在进入订单前返回错误；为失败另建持久审计事件需要新的 Gate A（不能把非订单事件伪装成 `order_audit`，也不能在本修订中顺手新增数据库结构）。在该后续设计完成前，生产卖出暂停闸不得解除。
...
2. 原始时间仅接受完整 `YYYY-MM-DD HH:MM:SS[.fraction]`，身份、代码、名称、价格和入场族逐项校验。未知策略族是显式数据缺口，不能并入 `Unknown` 后继续宣称全策略结果。
...
4. 已验证的十五分钟 K 线必须按时间严格升序、时间有效、OHLC/价格有限且大于零。信号早于首根、晚于末根或不在当前已发布的 bar 边界内时保持“未对齐”，不得用 `saturating_sub` 映射到首根。
5. 每个窗口从信号时刻之后的完整 4/16 根路径计算：终点收益、路径最大有利变动 MFE 和路径最大不利变动 MAE。聚合值使用逐事件指标，不再把跨样本最高/最低终点收益标为 MFE/MAE。
6. 卖出行不进入 R-12 上涨率。报告固定声明“事件上涨率不是完整策略胜率”；完整策略结论必须等待经济持仓归因、成本、基准和右删失处理完成。

### 10.3 失败与历史异常

固定日期 `2026-07-14..16` 和 `price < 1` 不是来源证据，必须从 R-12 删除。历史坏记录不能按硬编码规则静默丢弃：缺失/非法成交价、不可解析时间、未知方向/策略族、重复身份或非法 K 线使数据集显式不可用，并报告失败原因。仅“窗口未来 K 线尚不完整”与“信号在已验证 K 线覆盖外”可作为有计数的未对齐/右删失状态，不计入分母。
...
测试必须覆盖：早于首根/晚于末根拒绝对齐、非法或乱序 K 线整批失败、卖出不进入入场统计、九个入场族映射、未知族显式失败、真实 `fill_price`、逐路径 MFE/MAE、完整窗口右删失、200 样本门和报告免责声明。R-12、策略族映射和文档分别独立提交；回滚使用 `git revert <sha>`，不得恢复静默硬删除或改写历史事实。
...
未知引用失败；Observed/Scenario 标签隔离；盈利/亏损/平衡分母；200 个闭环和 84 天门。
