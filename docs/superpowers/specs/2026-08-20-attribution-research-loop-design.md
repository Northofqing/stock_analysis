...
测试必须覆盖：早于首根/晚于末根/午休/收盘后/非边界分钟拒绝对齐、两种稳定栅格、内部缺口或切换栅格整批失败、非法或乱序 K 线整批失败、公开未来路径缝独立拒绝非有限/非正/关系矛盾的 OHLC、卖出不进入入场统计、九个入场族映射、未知族显式失败、真实 `fill_price`、high/low 逐路径 MFE/MAE、完整窗口右删失、全未对齐/右删失计数仍进入审计、200 样本门和报告免责声明。R-12、策略族映射和文档分别独立提交；回滚使用 `git revert <sha>`，不得恢复静默硬删除或改写历史事实。
