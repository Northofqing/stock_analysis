...
## Addendum: BR-131 北交所涨跌停代码段完整性（2026-07-19）

- 根因：monitor 的涨停幅度推断只识别历史 `8/4` 北交所前缀，遗漏仓库其他模块已经登记的当前 `92xxxx` 代码段，使该代码段错误落入普通主板 10% 分支并污染连板/涨停证据。
- 修正：沿用现有 ST 5%、创业板/科创板 20%、北交所 30%、普通 A 股 10% 的优先级，只把 `92` 加入北交所分支；不修改配置或数值阈值。`cfg(test)` 仅剥离强制的 `TEST_CODE_` 身份前缀，使同一生产函数接受隔离测试身份。
- 验证与回滚：覆盖 `600/300/688/830/920` 和 ST 优先级，运行全 monitor、严格 Clippy 与设计一致性检查。整体回退 BR-131 文档、分支和测试；不得只删除 `92` 回归测试后保留错误 10% 口径。

## Addendum: Gate-D Task 22 核心真实来源失败边界（2026-07-19）

- 数据流：真实 provider 的生产入口、主备源顺序、HTTP/TCP transport 与严格解析器保持不变；本批不加入生产 mock、缓存兜底或默认业务值。单元测试使用仅测试进程可见的不可达代理客户端，执行现有生产请求构造与“所有真实来源失败”路径，并要求显式错误。
- 成功证据：已抽离的 JSON/CSV/帧解析器继续用完整本地协议夹具覆盖成功路径和坏数据拒绝；不伪造成功 HTTP transport。pipeline 只执行空批次、明确获取失败和已解析 `TEST_CODE_` 批次，不触发真实通知、真实账户读取或订单。
- 失败模式：不可达代理设置毫秒级 connect/request timeout，不得依赖公网状态。任一来源失败被降级为 `Ok(empty/default)`、任一缺失字段被补零，或测试触发外发/真实代码下单，Gate B 立即失败并返回本节重新设计。
- 旧模块与回滚：采用现有 `reqwest::Client` 注入、BR-092/BR-115 完整批次解析和 pipeline 的 `cfg(test)` 解析后数据槽；拒绝新增 provider trait fake、成功响应 HTTP stub、真实网络集成测试和默认 fallback。整体回退 Task-22 提交；若提取内部确定性 helper 则同时恢复原内联调用。

## Addendum: Gate-D Task 23 产业链已解析事实执行边界（2026-07-19）

- 数据流：`run_chain_analysis` 继续按原顺序从真实概念缓存/工具、东财板块与成分、真实持仓、龙虎榜和宏观来源取得完整事实；取得后的聚类落库、板块代码匹配、候选装配、无模型降级和报告组装移动到私有 resolved helper。生产入口与测试调用同一 helper，不新增来源 trait、provider fake、HTTP stub 或默认事实。
- 完整性与失败：聚类落库仍要求数据库已初始化、计数可转换且事务成功；概念板块必须按现有精确/去后缀/子串/登记别名顺序命中，候选只能来自已完成真实成分批次。任何缺失仍返回 `Err`，不得把未匹配板块、缺龙虎榜/持仓或模型失败改写为伪造结论。
- 测试隔离：唯一 `TEST_CODE_` 涨停股、概念、候选、持仓诊断和龙虎榜值直接进入 resolved helper，验证数据库生命周期、匹配优先级和无 API Key 时的显式“仅聚类”报告；不访问公网、不读取本人的持仓库、不发送通知或订单。
- 旧模块与回滚：保留 `DatabaseManager`、`cluster_by_concept`、`resolve_concept_alias`、所有 fetcher、LLM 提示与报告格式，只移动既有语句到内部函数。整体回退 Task-23 重构和测试即可；不得只留下测试入口而复制第二套生产逻辑。

## Addendum: Gate-D Task 24 真实 HTTP 尝试结果决策边界（2026-07-19）

- 数据流：东方财富与腾讯 provider 继续使用既有真实主机、URL、请求头、超时、重试次数和来源优先级。transport 取得 HTTP 状态和完整正文后，把这两个真实事实交给模块私有决策函数；该函数调用原严格协议解析器并返回“完整数据 / 可重试失败 / 不可重试失败”，生产循环只负责日志、退避和换主机。
- 完整性与缺失：2xx 空正文、非 JSON、缺字段或 BR-092/BR-097 校验失败都是可观测失败，禁止返回空行情或默认名称；4xx 保持不可重试，5xx 保持可重试。股票名称必须来自完整 `data.f58` 非空字符串，坏协议不得制造名称。
- 测试隔离：本地状态码和 JSON 文本直接执行生产同一决策/解析函数，覆盖成功、空正文、坏 JSON、4xx、5xx、短批次和坏行情；`TEST_CODE_` 仅用于领域身份，原生六位代码仅作为 provider 协议路由参数。不监听端口、不访问公网、不写数据库或发送订单。
- 旧模块与回滚：采用现有 `HttpProvider`、`GtimgProvider`、`ProviderError` 和严格解析器，不新增 provider fake、HTTP stub、缓存 fallback 或第二套解析。整体回退 Task-24 helper 和调用替换；不得只恢复 HTTP 内联分支而删除 BR-092/BR-097 校验。

## Addendum: Gate-D Task 25 RustDX 已校验日线装配边界（2026-07-19）

- 数据流：RustDX 真实 TCP 分页、panic 隔离和 BR-092 完整批次解析保持原入口；解析成功的日线与可空的真实腾讯实时行情进入私有装配函数，继续执行停牌缺口登记、涨跌停标记、60 日 Sharpe 和最新行可空指标补充。盘中价只写 `intraday_price`，不得覆盖 settled `close`。
- 失败与缺失：RustDX 空批次/分页错误仍阻断；腾讯来源错误或空行情只保留已验证日线并记录来源不可用，不补价格或指标。已有日线指标优先于实时可空指标，缺失保持 `None`。
- 测试与回滚：本地 `TEST_CODE_` 日线批次分别传入完整真实协议报价、空报价和显式来源错误，覆盖装配分支，不建立 TCP/HTTP 连接。整体回退 resolved helper 与调用移动；不得回退 RustDX 批次校验或恢复盘中价污染收盘价。

## Addendum: Gate-D Task 26 产业链检索结果确定性边界（2026-07-19）

- 数据流：盘后催化和主线新闻继续调用现有真实 `SearchService`，龙虎榜继续调用现有 `LhbDataFetcher`。已返回的搜索结果只通过私有函数执行标题去重、字符截断、总数上限和 Markdown 组装；模型返回的搜索词只通过私有协议函数清理编号、长度和句子泄漏。龙虎榜完整记录通过私有映射函数校验代码、有限净额和重复键。
- 规则保持：TOP 主线最多 5 条、催化总结果最多 10 条、主线新闻标题前 20 字去重与总数 10 条、搜索词总数最多 4 条的既有排序/限制不变（沿用已登记 BR-114）；缺搜索服务仍返回空章节，不生成新闻或龙虎榜事实。
- 测试与回滚：`TEST_CODE_` 搜索结果、模型文本和龙虎榜记录直接执行生产同一 helper，覆盖去重、截断、上限、缺日期、坏词、坏记录和重复记录，不调用公网或模型。整体回退 helper 与调用移动；不得放宽真实来源校验或引入静态新闻。

## Addendum: Gate-D Task 27 资金流与分时 HTTP 完整响应边界（2026-07-19）

- 数据流：资金流日线和当日分时继续按原三个 Eastmoney push2his 主机顺序请求。每次 transport 完成后，HTTP 状态和完整正文进入模块私有解析函数；资金流函数直接调用既有完整行解析器，分时函数返回严格 JSON 后继续调用既有 `build_intraday_shape`。
- 失败模式：非 2xx、读取失败、空/HTML、坏 JSON、缺 `data.klines` / `preClose` / `trends`、坏 OHLCV、时间缺口或 ±20% 异常均为该真实主机失败并进入下一真实主机；全部失败必须返回聚合错误。禁止返回默认 `MoneyFlowSummary` / `IntradayShape` 或补零。
- 测试与回滚：本地状态码和正文覆盖完整成功及每个失败类别，不启动网络或写数据库。整体回退 response helper 与调用移动；不得回退 BR-115 严格形态校验或改变主机/字段参数。

## Addendum: Gate-D Task 28 公告 HTTP 完整响应与正文边界（2026-07-19）

- 数据流：公告列表与高风险公告正文继续请求既有东方财富真实接口。transport 完成后，HTTP 状态和完整正文进入模块私有解析函数；列表函数复用既有逐行完整性校验，正文函数要求 `data.content` 为非空文本。标题分类、正文选择和最终公告装配语义不变。
- 失败模式：非 2xx、正文读取失败、空/HTML/坏 JSON、缺 `data/list/content`、坏代码、坏日期或空标题均显式失败；任一应取正文的高风险公告缺正文时整批失败。不得以标题、摘要或空串伪装正文成功，也不得在生产路径加入固定公告。
- 测试与回滚：本地状态码与正文直接执行生产同一解析函数，覆盖列表/详情完整成功以及每类失败，不建立网络监听、不写数据库。整体回退 response helper 与 transport 调用移动；不得回退既有 BR-059 标题分类、详情选择和严格批次装配。

## Addendum: Gate-D Task 29 行业对标 HTTP 与跨页决策边界（2026-07-19）

- 数据流：行业名、二级行业映射和成份股仍来自既有三个东方财富真实主机。每次 transport 完成后，HTTP 状态与完整正文进入私有严格解析函数；解析后的行业映射页通过私有跨页合并函数拒绝冲突，最终以真实行业名、完整映射和完整成份批次构建对标。
- 失败模式：非 2xx、读取失败、空/HTML/坏 JSON、缺行业名、缺映射、跨页名称冲突、空成份批次或坏估值字段都显式失败。进程缓存只能保存完整非空映射，不得缓存部分页、用代码猜行业或补造同业指标。
- 测试与回滚：本地状态码、正文、映射页和 `TEST_CODE_` 成份事实执行生产同一 helper；真实 transport 只以不可达客户端验证全源失败。整体回退 helper 与调用移动；不得恢复忽略 HTTP 状态、部分页提交或默认行业。

## Addendum: Gate-D Task 30 小型核心状态与缓存提交边界（2026-07-19）

- 数据流：排除板块仍由真实板块排名与成份来源取得，私有 helper 只负责同日缓存命中和已返回成份的原子映射；龙头 BOM 仍由真实分析器取得，私有 helper 只构造提示并应用已返回协议文本。资金/分时/财务缓存仍调用现有真实 provider，只有已成功返回且完成校验的对象可以写缓存；产业链主线提示仍读取现有 `chain_daily`。
- 失败模式：排除板块缺失/成份失败不得制造命中；BOM 不可用保持 `None`；空资金流、未 present 分时、provider 错误或数据库错误不得写缓存/拼成有效证据。同一缓存日的构建器只执行一次，失败事实不得由默认值替代。
- 测试与回滚：使用本地 `TEST_CODE_` 持仓、板块成份、BOM 文本、已解析资金/分时和 `ChainDailyRow` 协议行；不可达客户端只验证真实 transport 失败，共享数据库不写入测试主线。整体回退 helper、调用移动与测试；不得把测试对象注册为生产 provider 或保留部分缓存写入。

## Addendum: Gate-D Task 31 仅测试回环 HTTP 协议边界（2026-07-19）

- 决策：Task 22 禁止用 HTTP stub 代替 provider 成功证据，目的是防止测试入口渗入生产来源。剩余覆盖缺口集中在真实客户端取得状态/正文和分页的 transport 代码，因此新增仅在 `cfg(test)` 编译的 `127.0.0.1` 单次响应回环器；它不实现 provider、不进入任何生产注册表，生产二进制中不存在。测试仍必须把领域身份标为 `TEST_CODE_`。
- 数据流：生产 host/base URL、请求头、参数、重试与来源顺序不变；已有私有 `*_from_hosts` / `*_with_client` seam 在测试中指向临时回环地址，真实 `reqwest` 完成连接、读取 HTTP 状态与完整正文，再调用生产同一严格解析器。回环器按预期请求数关闭，未完成请求显式失败。
- 范围与回滚：首批覆盖产业链板块分页/成份、公告列表/详情和行业名/映射/成份的完整 transport；不用于订单、账户、通知或生产成功 fallback。整体回退 `cfg(test)` 回环器、私有 base/host 参数和测试；不得删除由回环测试发现的状态、批次或原子性校验。

## Addendum: Gate-D Task 32 东方财富、腾讯与新浪回环 transport（2026-07-19）

- 数据流：三个 provider 继续使用既有真实主机、URL、请求头、来源顺序和重试次数。仅把现有 transport 循环下沉到模块私有的 host/base seam；生产入口传入原常量，`cfg(test)` 测试传入 Task 31 的本机回环地址。真实 `reqwest` 必须完成请求、状态读取和完整正文读取，再进入既有严格解析与 BR-092/BR-125 批次校验。
- 完整性与失败：东方财富 4xx 仍为终止失败，网络/5xx/正文/协议失败仍按原顺序重试；腾讯日线必须在真实实时行情可用时只补充可空盘中字段，任何日线协议错误仍阻断；新浪日线协议缺少成交额时继续显式不可用，不得用 0 填充。名称、实时行情、日期、价格、成交量、成交额和来源时间缺失均不得制造成功事实。
- 测试隔离：回环服务只在单元测试构建存在，领域对象使用 `TEST_CODE_` 身份，原生六位代码仅限 provider 路由参数。测试不得访问公网、写真实数据库、注册测试 provider、发送通知或订单；回环响应只用于执行生产同一协议解析器，不能成为生产 fallback。
- 旧模块与回滚：采用现有 `HttpProvider`、`GtimgProvider`、`SinaProvider`、Task 31 回环器和严格解析器，不改变 provider 注册与优先级。整体回退三个私有 seam 与测试；不得回退严格批次校验、恢复缺失字段补零或改变生产端点。

## Addendum: Gate-D Task 33 BaoStock TCP 会话与流水线测试隔离（2026-07-19）
...
## Addendum: Gate-D Task 35 核心编排与真实 SQLite 剩余边界（2026-07-19）

- 数据流：回测入口仍通过真实 `DataFetcherManager` 获取日线和基准；仅在 `cfg(test)` 构建复用流水线已有的 `test_fetched_data` 槽，使排序、页数、空批和来源失败编排可由 `TEST_CODE_` 完整批次执行。生产构建不存在测试槽分支，报告与交易/NAV 审计仍由现有入口强制落盘。
