...
公告列表协议可增加仓库内已有、严格解析并保留 provenance 的真实备用协议；备用 JSONP 只接受登记的 `jQuery(...)` 包装，裸 JSON、其他 callback、HTML 或不完整载荷都返回错误。批次必须保存 provider、endpoint、查询日期、观察时间、最终协议和主协议失败事实，协议选择及逐详情拒绝在消费前写入不可篡改审计；每个详情失败保留真实失败观察时间、结构化原因和可重试性，不能统一抹成一种错误。任一公告详情失败只拒绝该公告。公告组件失败时，其他组件只有在各自来源、时间和字段完整时才可独立形成报告，正文必须显示公告能力不可用。

真实持仓组件只接受携带 provider、不可变 batch identity 和源时间的券商逐持仓批次，并执行 30 秒 freshness gate。当前尚未接入该合同，因此 R-08 必须将真实持仓组件标为不可用；不得读取 `stock_position.updated_at` 或数据库写入时间冒充券商证据，也不得把未知持仓关系写成“非持仓”。虚拟观察目录或快照缺失是组件不可用，不是完整空集合。虚拟观察、隔夜事件和公告等独立完整组件仍可形成显式降级报告；四个组件均不可用时必须失败。
...
- 复盘审计使用按日期分片的 SHA-256 hash-chain JSONL，跨进程锁覆盖验链、批量追加和 `flush + sync_data`；非空旧文件必须以换行结束，否则作为半条记录拒绝续写；测试与生产路径即使使用同一 override base 也强制进入独立命名空间，且不配置少于五年的删除策略；
- task transition 记录 `status/success/snapshot_size/retryable/next_attempt/reason_code/identity_hash`，原 `success` 仅由 `Delivered` 派生；`observed_at` 是本地真实观察时间，`source_time` 只在上游真实提供时记录，缺失必须保持空；候选拒绝和公告协议选择记录独立强类型 payload；
