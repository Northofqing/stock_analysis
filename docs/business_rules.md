...
| BR-006 | 过滤 | 基于真实胜率关停 0% 主题。`chain_rules.toml` 中 `enabled = false` 的规则在 `chain_mapper::map_chain_rules` 阶段被过滤, 后续推送不再命中。当前关停清单 (14 天回测, **真 0% 胜率**): AI硬件-液冷 (6/0), 半导体-先进封装 (12/0), 消费电子 (6/0), 新能源-电池 (10/0 历史归类兜底), 稀土永磁 (15/0), 稀有金属 (89/1 仍差, 留观察)。全部 6 个 enabled=false, winrate_simulator 自动从 config 读取。**BR-006 决策修订 (v9.2 PR-3 数据回灌后)**: 早期一轮关停的 6 个主题 (半导体-制造代工 / 创新药-CXO / AI硬件-MLCC / Rubin / AI算力 / AI硬件-CPO) 在 PR-2 修复 R-3 数据断层后, 真实胜率显现非 0%, 已重新开启并按真实胜率加权 (半导体-制造代工 64.6% → priority 88, 创新药-CXO 36.8% → priority 85)。教训: 早期 0% 是数据缺失假象, 不是真信号, BR-006 触发必须以 "真 verify 完成的样本" 为前提。再启条件: 跑 `winrate_simulator` 验证 ≥10% 胜率。 | `src/opportunity/chain_mapper.rs::map_chain_rules` + `config/chain_rules.toml` | `src/opportunity/chain_mapper.rs::test_br006_disabled_chains_excluded` + `src/bin/winrate_simulator.rs` | 2026-06-29 |
