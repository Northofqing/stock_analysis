...
| kind | 状态 | producer | Unit | 证据符号 | 说明 |
| --- | --- | --- | --- | --- | --- |
| CloseCall | ACTIVE | close-call, startup-resume-close-call | MU-close-call | src/bin/monitor/durable_delivery_runtime.rs::AuthoritativeSinkPort for MagiclawAuthoritativeSink; src/bin/monitor/durable_delivery_runtime.rs::durable_kind_and_sub_kind_with_override; src/bin/monitor/durable_delivery_runtime.rs::ensure_startup_reconciled; src/bin/monitor/durable_delivery_runtime.rs::reconcile_startup_blocking; src/bin/monitor/main.rs::main; src/bin/monitor/main.rs::monitor_loop; src/bin/monitor/main.rs::prepare_close_call_messages; src/bin/monitor/notify.rs::push_counted_with_binding; src/durable_delivery/coordinator.rs::begin_attempt; src/durable_delivery/coordinator.rs::build_reconcile_summary; src/durable_delivery/coordinator.rs::inspect_business_date_once_claim; src/durable_delivery/coordinator.rs::reacquire_rejected; src/durable_delivery/coordinator.rs::reconcile_all_pending; src/durable_delivery/coordinator.rs::recover_one_expired_attempt; src/durable_delivery/coordinator.rs::resume_deliverable; src/durable_delivery/model.rs::DeliveryEnvelope; src/durable_delivery/model.rs::compiled_policy_catalog | t≥14:55 counted per ticket，只有下界；零条也封close_call_pushed，失败保留。 另有普通启动all-date既存immutable envelope恢复producer，归原durable owner/Unit；不受正常loader/业务时窗/backfill最近5日限制，不生成新claim。 |
...
| HoldingEvent | INACTIVE |  |  | src/bin/monitor/durable_delivery_runtime.rs::AuthoritativeSinkPort for MagiclawAuthoritativeSink; src/bin/monitor/durable_delivery_runtime.rs::durable_kind_and_sub_kind_with_override; src/bin/monitor/durable_delivery_runtime.rs::ensure_startup_reconciled; src/bin/monitor/durable_delivery_runtime.rs::reconcile_startup_blocking; src/bin/monitor/main.rs::main; src/bin/monitor/main.rs::monitor_loop; src/bin/monitor/main.rs::reject_unbound_alert_delivery; src/durable_delivery/coordinator.rs::begin_attempt; src/durable_delivery/coordinator.rs::build_reconcile_summary; src/durable_delivery/coordinator.rs::reacquire_rejected; src/durable_delivery/coordinator.rs::reconcile_all_pending; src/durable_delivery/coordinator.rs::recover_one_expired_attempt; src/durable_delivery/coordinator.rs::resume_deliverable; src/durable_delivery/model.rs::DeliveryEnvelope | 新claim/新producer审计：无生产发送caller；legacy summary停在counted binding不可用，reject_unbound_alert_delivery明确sink_calls=0；renderer/registry/fixture不构成producer。 既存decision恢复边界：runtime映射为durable HoldingEvent/None。普通启动all-date barrier仍可从已存immutable envelope恢复Reserved或已获retry授权的RejectedDurable；不受正常loader/任务时窗/最近5日backfill限制，不重取当前source，foreign lease/Uncertain不盲发。仅说明历史counted decision的可恢复能力，未查询其存在性；状态保持INACTIVE且producer_ids为空，不造活动producer/Unit。 |
...
触发：普通启动完成core DB绑定后，ensure_startup_reconciled形成all-date startup barrier，早于常驻任务、--review和--push；扫描delivery_decisions全部日期，不受正常loader、业务发送时窗、交易日门或backfill最近5日/8-task集合限制。显式P01 compensation独占命令跳过全局barrier。 证据：src/bin/monitor/main.rs::main；src/bin/monitor/durable_delivery_runtime.rs::ensure_startup_reconciled；src/bin/monitor/durable_delivery_runtime.rs::reconcile_startup_blocking；src/durable_delivery/coordinator.rs::reconcile_all_pending；src/durable_delivery/coordinator.rs::build_reconcile_summary。

输入：只读取已存immutable envelope的原business_date、kind/sub_kind、scope、occurrence、source binding、policy version、rendered content/hash及关联decision；从stored envelope构造attempt request，不调用当前行情、账户、新闻、review loader/provider/LLM，不以新source替换原载荷。 证据：src/bin/monitor/durable_delivery_runtime.rs::durable_kind_and_sub_kind_with_override；src/durable_delivery/model.rs::DeliveryEnvelope；src/durable_delivery/coordinator.rs::resume_deliverable；src/durable_delivery/coordinator.rs::begin_attempt。

权威事实：发送权限仅来自既存decision/state及reservation/claim、retry_authorized、lease/fence；reconcile先完成本地append/payload固定点，再对deliverable decision直接begin_attempt/sink。已存authority不表示当前来源已重新验证，也不证明本环境存在这类历史记录。 证据：src/durable_delivery/coordinator.rs::reconcile_all_pending；src/durable_delivery/coordinator.rs::build_reconcile_summary；src/durable_delivery/coordinator.rs::resume_deliverable；src/durable_delivery/coordinator.rs::begin_attempt；src/durable_delivery/coordinator.rs::reacquire_rejected；src/durable_delivery/coordinator.rs::recover_one_expired_attempt；src/bin/monitor/durable_delivery_runtime.rs::AuthoritativeSinkPort for MagiclawAuthoritativeSink。

策略：仅Reserved及retry_authorized=true的RejectedDurable可进入恢复；Rejected需重新取得原claim/rolling head与所需预算reservation，不在启动中新增retry授权。begin_attempt要求Reserved和唯一authoritative sink；原rendered content/hash直接送namespace绑定sink。未过期foreign lease和UncertainManualReview保留，不盲发；过期AttemptInFlight撤销fence并转Uncertain而非自动重发。每个decision启动内最多恢复一次，仍deliverable或本地固定点失败则阻止新producer激活。 review终态的既存task transition经date/task/hash核验后hydrate并ack；原业务日通知owner不变。 证据：src/durable_delivery/coordinator.rs::reconcile_all_pending；src/durable_delivery/coordinator.rs::build_reconcile_summary；src/durable_delivery/coordinator.rs::resume_deliverable；src/durable_delivery/coordinator.rs::begin_attempt；src/durable_delivery/coordinator.rs::reacquire_rejected；src/durable_delivery/coordinator.rs::recover_one_expired_attempt；src/bin/monitor/durable_delivery_runtime.rs::AuthoritativeSinkPort for MagiclawAuthoritativeSink；src/bin/monitor/review_batch.rs::apply_durable_hydrations_with_evidence；src/bin/monitor/review_batch.rs::ReviewTask；src/bin/monitor/main.rs::apply_durable_review_hydrations_and_acknowledge；src/bin/monitor/review_batch.rs::ReviewScheduleState；src/bin/monitor/main.rs::post_session_review_scheduler；src/bin/monitor/main.rs::run_review_only。
...
策略：仅Reserved及retry_authorized=true的RejectedDurable可进入恢复；Rejected需重新取得原claim/rolling head与所需预算reservation，不在启动中新增retry授权。begin_attempt要求Reserved和唯一authoritative sink；原rendered content/hash直接送namespace绑定sink。未过期foreign lease和UncertainManualReview保留，不盲发；过期AttemptInFlight撤销fence并转Uncertain而非自动重发。每个decision启动内最多恢复一次，仍deliverable或本地固定点失败则阻止新producer激活。 证据：src/durable_delivery/coordinator.rs::reconcile_all_pending；src/durable_delivery/coordinator.rs::build_reconcile_summary；src/durable_delivery/coordinator.rs::resume_deliverable；src/durable_delivery/coordinator.rs::begin_attempt；src/durable_delivery/coordinator.rs::reacquire_rejected；src/durable_delivery/coordinator.rs::recover_one_expired_attempt；src/bin/monitor/durable_delivery_runtime.rs::AuthoritativeSinkPort for MagiclawAuthoritativeSink。
...
触发：普通启动完成core DB绑定后，ensure_startup_reconciled形成all-date startup barrier，早于常驻任务、--review和--push；扫描delivery_decisions全部日期，不受正常loader、业务发送时窗、交易日门或backfill最近5日/8-task集合限制。显式P01 compensation独占命令跳过全局barrier。 本入口不创建R03新claim；不经过LegacyAccountGate。 证据：src/bin/monitor/main.rs::main；src/bin/monitor/durable_delivery_runtime.rs::ensure_startup_reconciled；src/bin/monitor/durable_delivery_runtime.rs::reconcile_startup_blocking；src/durable_delivery/coordinator.rs::reconcile_all_pending；src/durable_delivery/coordinator.rs::build_reconcile_summary；src/bin/monitor/push_templates.rs::dispatch_post_session_review。

输入：只读取已存immutable envelope的原business_date、kind/sub_kind、scope、occurrence、source binding、policy version、rendered content/hash及关联decision；从stored envelope构造attempt request，不调用当前行情、账户、新闻、review loader/provider/LLM，不以新source替换原载荷。 本producer只消费已存IndustryChain/None的R03信封。 证据：src/bin/monitor/durable_delivery_runtime.rs::durable_kind_and_sub_kind_with_override；src/durable_delivery/model.rs::DeliveryEnvelope；src/durable_delivery/coordinator.rs::resume_deliverable；src/durable_delivery/coordinator.rs::begin_attempt。

权威事实：发送权限仅来自既存decision/state及reservation/claim、retry_authorized、lease/fence；reconcile先完成本地append/payload固定点，再对deliverable decision直接begin_attempt/sink。已存authority不表示当前来源已重新验证，也不证明本环境存在这类历史记录。 既存R03 claim不依赖当前auto/manual临时任务状态授予发送权限。 证据：src/durable_delivery/coordinator.rs::reconcile_all_pending；src/durable_delivery/coordinator.rs::build_reconcile_summary；src/durable_delivery/coordinator.rs::resume_deliverable；src/durable_delivery/coordinator.rs::begin_attempt；src/durable_delivery/coordinator.rs::reacquire_rejected；src/durable_delivery/coordinator.rs::recover_one_expired_attempt；src/bin/monitor/durable_delivery_runtime.rs::AuthoritativeSinkPort for MagiclawAuthoritativeSink；src/durable_delivery/coordinator.rs::inspect_business_date_once_claim。

策略：仅Reserved及retry_authorized=true的RejectedDurable可进入恢复；Rejected需重新取得原claim/rolling head与所需预算reservation，不在启动中新增retry授权。begin_attempt要求Reserved和唯一authoritative sink；原rendered content/hash直接送namespace绑定sink。未过期foreign lease和UncertainManualReview保留，不盲发；过期AttemptInFlight撤销fence并转Uncertain而非自动重发。每个decision启动内最多恢复一次，仍deliverable或本地固定点失败则阻止新producer激活。 ReviewTask::ALL/from_label包含R03；持久task transition按匹配schedule date、task identity和hash应用，再ack，可关联auto常驻state与manual临时audit；foreign日期仍pending。 证据：src/durable_delivery/coordinator.rs::reconcile_all_pending；src/durable_delivery/coordinator.rs::build_reconcile_summary；src/durable_delivery/coordinator.rs::resume_deliverable；src/durable_delivery/coordinator.rs::begin_attempt；src/durable_delivery/coordinator.rs::reacquire_rejected；src/durable_delivery/coordinator.rs::recover_one_expired_attempt；src/bin/monitor/durable_delivery_runtime.rs::AuthoritativeSinkPort for MagiclawAuthoritativeSink；src/bin/monitor/review_batch.rs::apply_durable_hydrations_with_evidence；src/bin/monitor/review_batch.rs::ReviewTask；src/bin/monitor/main.rs::apply_durable_review_hydrations_and_acknowledge；src/bin/monitor/review_batch.rs::ReviewScheduleState；src/bin/monitor/main.rs::post_session_review_scheduler；src/bin/monitor/main.rs::run_review_only。
...
| startup-all-pending | src/durable_delivery/coordinator.rs | reconcile_all_pending | 3735–3766 | f3c83c0431971edc0740a44fe528cde98962fae39e66bac9be4bb9eb0a939aff |
...
| startup-begin-attempt | src/durable_delivery/coordinator.rs | begin_attempt | 4793–4941 | ad070e3320996684e2dcce99f936fc44a2d309998accfb3e53eb63f10d0b229d |
| startup-expired-attempt | src/durable_delivery/coordinator.rs | recover_one_expired_attempt | 5233–5339 | cc37d93adedd661de031fe08d4bfb1775b3a422a924ecf47520ac6b08eae72dc |
...
| startup-list-deliverable | src/durable_delivery/coordinator.rs | build_reconcile_summary | 5865–5976 | 2372254ffb0eb37695a0d4daab0a3dc48ce46b19cd9fd302664473cdd76d79d6 |
| startup-reacquire | src/durable_delivery/coordinator.rs | reacquire_rejected | 4736–4791 | 2d896a192637b3359b0454adf55ef484c00d630cfc9ea8ebe9f8aa9f731fad7a |
...
| startup-resume | src/durable_delivery/coordinator.rs | resume_deliverable | 3624–3673 | a2920da27c547667eafe9f399b48163a2a6248f6b19460e0663ac373e373aa38 |
