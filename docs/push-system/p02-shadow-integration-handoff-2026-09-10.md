...
2026-09-11范围调整：本次按[单用户本地模式](single-user-local-scope-2026-09-11.md)提供实际本地注册/context，不再等待外部可信身份或认证broker。下面的旧认证前置按该变更排除；同次事实/完整提案比较、真实dispatcher接线、输入有效性与防重复效果要求仍适用，不得用缺量比或测试常量冒充真实数据。
