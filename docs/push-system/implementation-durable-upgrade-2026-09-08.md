...
2026-09-10并行只读核查补充：问题不仅是只读取了单个 decision。当前 [enqueue_audit](../../src/durable_delivery/coordinator.rs#L8583)仍以目标 decision 的最大 rowid 选择前驱；[v5–v9 升级分支](../../src/durable_delivery/schema.rs#L135)不会再次经过已修复的 v4→v5 保序复制。下面是前驱合同允许的最小偏序反例，不是生产数据或已运行的新增 Rust 测试：

```text
r(D, NULL) → x(D, r)
b(E, NULL) → y(D, b)

合法顺序一：r, x, b, y     D 的最后一项是 y
合法顺序二：r, b, y, x     D 的最后一项是 x
```

两种顺序满足相同的跨 decision 前驱图，却有不同的 D 尾节点；因此遍历全图也不能总是找回原始追加顺序。时间戳可相同，hash 仅绑定内容，不补充这条先后信息。[既有 W16 正例](../../src/durable_delivery/tests.rs#L9432)使用真实冻结 schema，在 INSERT 时建立跨 decision 依赖，保留原不可变记录，并要求 global 恢复最终完成两个 decision；兼容实现不得删除该正例或统一拒绝多段来通过测试。

据此将后续任务拆成两个不同的验收条件，而不是选一个任意尾：

- 若目标 decision 有一个节点，其全局前驱祖先覆盖该 decision 的其余全部节点，图本身可给出唯一最大节点；节点自身不属于其严格祖先。Pending 也必须参与，不能只看已追加记录。这是可验证的算法条件，当前尚未实现。
- 若存在多个不可比较的最大节点，必须取得额外、可信且覆盖完整 cutoff（包括 Pending）的顺序/迁移来源证据，或另行明确记录历史不确定性的恢复合同。当前非空 append 引用、数据库版本和“看起来有序”的 rowid 均不提供该证明；本轮没有选定或实现新的生产恢复合同。

对新建/已保序库，也不能凭 caller 声明将 rowid 升为权威来绕过旧库问题；将来任何 provenance 必须由实际创建/迁移路径建立并持久绑定。以上核对使后续兼容设计的二义性条件明确，不构成兼容修复完成，更不授权迁移或修写生产库。

### 新来源设计与运行期版本前置防护

2026-09-10 独立设计核对基于 `8daa8bf2d8490807256a5bb6ed92b0a4ea8a5734`，结论见[限定设计证据](../../.planning/2026-09-10-durable-audit-compatibility/findings.md)。没有开生产库，也没有实现 v10：

- **保住本次复制顺序不等于证明输入历史可靠。** [v3→v4](../../src/durable_delivery/schema.rs#L886)重建时没有显式携带 rowid 或指定复制顺序；因此仅凭输入为 v4，或后来执行了已修复的 v4→v5，不能给历史记录授予可靠原序身份。`77cc3bc` 的已完成有限修复不重开，也不扩大其结论。
- **拟议的持久来源必须覆盖真实 INSERT 与 Pending。** 若采用新增 v10 sidecar，应由正式初始化/迁移事务和实际 outbox INSERT trigger 记录新序号；旧非空历史保留未知来源，不能按当前 rowid 补造旧序号。正式重启后仍需检验完整成员、绑定及 trigger，不能自动补表补 trigger 后声称丢失区间已恢复。此方案待完整合同后实施，不是当前 schema 的能力。
- **不能静默改变旧历史恢复规则。** 多个不可比较最大节点需要完整可信外证，或明确定义“历史缺口接续”新语义。已向用户询问这项实质选择，当前未收到回答；全局恢复的失败粒度也尚未定案。不可先插桥接事件再把未知前缀说成已消歧。
- **运行期版本检查是独立必要前置，不是旧 writer 撤销。** 当前版 open 拒绝高版本，但已打开实例的通用 wrapper 未阻止版本漂移，持久引用校验还会因版本不匹配而跳过。已拆出[运行期版本防护计划](../superpowers/plans/2026-09-10-durable-runtime-schema-guard.md)；进展及验证见[专项记录](implementation-durable-runtime-schema-guard-2026-09-10.md)。修复新版程序不能改变已经部署的旧二进制；正式旧 writer 排空/切换仍需实际证据与授权。
- **恢复过程不是一次可回滚的外部事务。** 当前 reconcile 先后处理审计 append、payload append 与最终化；仅在 enqueue 检查未知尾不能撤销已经成功的外部 append。后续来源方案需先规定效果前预检、事务内重验与并发失败语义，不能承诺现有接口做不到的全调用原子性。

完整 W15/W16/W17/W19、52 Unit 迁移及发布门禁继续按[当前证据入口](README.md)推进。离线蓝图/双 HTML、统一 checker 与本地 CI 配置接线已分别交付，见[蓝图记录](implementation-current-blueprint-2026-09-10.md)及[统一门禁记录](implementation-unified-document-checker-2026-09-09.md)；真实远端 CI 与生产发布验收仍未取得，不重复将本地接线列作未开发。原有限迁移修复范围见[本批计划](../superpowers/plans/2026-09-08-durable-audit-logical-tail.md)。
