...
PROVISIONAL；追加基点 `5ab013c`，沿用 `07781bf` 源码。下表38行按入口/形态分行，保留受阻和缺源入口；不将业务成交、快照写入、`Ok`、日志或 `Pushed` 字样升级为 TransportAccepted。补充简写：DM=`src/durable_delivery/model.rs`；DR=`src/bin/monitor/durable_delivery_runtime.rs`；DC=`src/durable_delivery/coordinator.rs`；DS=`src/durable_delivery/schema.rs`；PS=`src/trading/paper_sell.rs`；TA=`src/decision/t0_advisor.rs`。S01–S25 是业务完成边界族，不宣称每族已有一个原子通知 owner；候选 Unit TS01–TS25 对应这些边界，族内分层和共享关系如下。

通用治理 G：N:2217 `push_governor_inner_with_source_evidence` / N:2877 `push_presented_v3` → VA:438 `v14_gate_with_sub_kind`，L4:123/161 `reserve_with_identity` / `commit_with_identity` 的 event.kind、业务 identity、sub_kind 冷却域；普通入口的 code 或空 code 与 source-fact event ID 不可混用。没有子类参数的同 kind/空 code 共享全局冷却。counted 治理 C：N:2891 `push_counted_with_binding` → DR 的 `CountedDeliveryBinding` / counted runtime，按 DM:613 `DeliveryEnvelope::new` 的 decision identity 绑定 occurrence、来源fingerprint、scope、subject、policy、正文hash；source canonical另存hash并验证，不能只拿可读 occurrence 字符串当去重主键。DM:409 `compiled_policy_catalog` 中 HoldingPlan/T0Advice 为 PerTicket/Rolling/1800s，CloseCall 为 PerTicket/Rolling/86400s；SectorTop/SectorAnomaly 为 Global/BusinessDateOnce/86400s。DS:264 `business_date_once_claims` 实际主键为 `(business_date,push_kind,sub_kind,scope_key)`，schedule_occurrence_identity属于其关联decision，并非claim主键第五列；DC:2848 `inspect_business_date_once_claim` 按这四列查。共享 coordinator、预算或数据表不等于共享 occurrence 完成。
...
| attribution-daily / AttributionDaily / ACTIVE / 盘后 | 15:05–15:20，epoch daily/window+当日收盘价，先persist并保存Markdown | epoch计算/完整性错误分类；G；落库和文件是分析产物 | S20：`ATTRIBUTION_LAST_RUN[date]`；AttributionDaily/空code冷却 | 计算/持久化/文件Err不封；取得text后任何发送outcome均封日，无receipt补偿 | TS20；M:9063–9141 `monitor_loop`，9118 send、9120 seal；`compute_epoch_daily` / `persist_epoch_daily` |
| g5b-attribution / G5bAttribution / ACTIVE / 盘后 | 同15:05–15:20但独立batch；今日告警top events→LLM assessment→append row→摘要 | LLM receipt是分析来源证据，非传输receipt；G | S21：`G5B_LAST_RUN[date]`；G5bAttribution/空code冷却，不按row.code传票键 | 空records封日；无provider continue不封；每条分析/append失败continue，整批末仍封日；发送outcome仅log，done也非接受计数 | TS21，与S20分开；M:9147–9238 `monitor_loop`；`append_deep_attribution_row` / `render_deep_attribution_summary` |
