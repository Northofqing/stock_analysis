...
| R08 | EventCalendar / ACTIVE / 盘后 / SourceOnly | T:10983/11022 `dispatch_r08_event_calendar_outcome` / with_loader：CNInfo公告、CFFEX交割、美国指数、USD/CNY；review日d与reminder下一交易日分开；T:11100 CFFEX GatewayError直接typed失败，其余分量允许显式degraded；Provider来源canonical+R08专用counted | RV07：Rolling EventCalendar/None/GLOBAL的decision和review occurrence `(d,kind,None,GLOBAL,t(d,R08))`，DM:442；auto/manual/backfill查询同durable task occurrence。不是BusinessDateOnce claim；R08可有0 item的Delivered hydration | typed永久/可重试按GW retryable；新获取失败未产生durable decision，auto仅内存终态，旧制品/跨重启限制见专项。部分完整组件可发，全部不可用Failed(true)，binding非法Failed(false)。T:6542允许R08 snapshot_size=0；不能以零日历事件推断无发送 |
...
| chain-preopen-timer / enum外 / ACTIVE / 盘前 | M:9404 09:05≤t<09:15，传true调用同chain；business_date为此前已完成交易日，timer key是当前calendar date | RV20：独立static `CHAIN_PREOPEN_LAST[calendar_date]`；函数Ok（包括send false/Err）才封日，source/分析/保存Err不封，窗口外无补偿。与RV19仅共用业务分析/文件，不共享durable通知cursor | M:9416 `monitor_loop` caller、AM:106；文件HHMM不构成receipt，不能按同函数与盘后timer合并 |
| chain-post-timer / enum外 / ACTIVE / 盘后 | M:8977 15:30≤t<15:35，业务日为当日已完成日，独立timer；同chain源/通知函数 | RV21：`CHAIN_POST_LAST[calendar_date]`，同Ok封日、发送false/Err仍封，pipeline Err保留重试；与RV20不同static、不同occurrence窗口；重启两者均丢失内存封日 | M:8988 `monitor_loop` caller、AM:176 send；09:05/15:30不是ReviewTask，也不共用ReviewScheduleState |
...
6. 单股先持久保存再send，false也“成功”且Err不阻Some；summary先保存后send，false/Err仍最终Ok；chain false/Err只warn仍Ok，两个timer据Ok封日。NS success_count>0不代表每个配置渠道成功，也没有durable TransportAccepted cursor；不从分析结果、文件或“已推送”日志升级通知authority。PA:1248/1255、PN:107/109、AM:176、M:8988/9416、NS:126/266。
...
| QR06 | CLI single/summary/chain及两个timer怎样用独立持久notification cursor关联精确输入、载荷和所有要求渠道的TransportAccepted，并恢复“已保存未送达”和不确定发送 | RV17–RV21、TR13–TR17；当前仅bool聚合/业务文件/内存门，缺真实receipt绑定；与Task5 QS05同类但owner不同 |
