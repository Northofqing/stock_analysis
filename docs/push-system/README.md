...
- [盘后产业链持久恢复实施计划](../superpowers/plans/2026-09-11-chain-post-close-recovery.md)：基线42ce098，按真实pipeline固定准备→同业务库检查点/发送进度→实际timer/startup→强cursor与六门禁推进。Task1仍在开发：旧入口已委托同一次固定准备；模型/检索及旧报告协议9项通过，另1项真实SQLite回归通过；后续核心失败7项、候选证据8项、版本化封存9项定向回归分别通过，详见[实施进度与验证边界](implementation-chain-preparation-2026-09-11.md)。剩余分支覆盖、旧测试迁移和最终审查仍待。存储只读设计已完成，不等于持久层代码完成；当前一名Rust实施者、主控单Cargo队列。尚未改定时器，不代表误封日已修复或生产已切换。
