...
mod tests {
    ...
    fn test_ok_position() {
        ...
        let positions: Vec<Position> = (1..=15)
            .map(|i| pos(&format!("TEST_CODE_{:06}", i), 10.0, 1000))
            .collect();
        let mut prices = std::collections::HashMap::new();
        for i in 1..=15 {
            prices.insert(format!("TEST_CODE_{:06}", i), 11.0);
        }
    ...
    fn test_zero_total_value() {
        // 所有已知价格为 0 → 总市值为 0，不进入比例计算。
        let positions = vec![pos("TEST_CODE_000001", 10.0, 1000)];
        let prices = std::collections::HashMap::from([("TEST_CODE_000001".to_string(), 0.0)]);
        let v = check_position_limits(&positions, &prices, &HardLimits::default());
        assert!(v.is_empty());
    }

    #[test]
    fn test_missing_price_and_sector_limit_are_explicit() {
        let mut first = pos("TEST_CODE_000001", 10.0, 1000);
        first.name = "有行情".into();
        first.sector = "测试板块".into();
        let mut missing = pos("TEST_CODE_000002", 10.0, 1000);
        missing.name = "缺行情".into();
        missing.sector = "测试板块".into();
        let positions = vec![first, missing];
        let prices = std::collections::HashMap::from([("TEST_CODE_000001".to_string(), 10.0)]);

        let violations = check_position_limits(&positions, &prices, &HardLimits::default());
        assert!(violations.iter().any(|item| item.rule == "缺价"
            && item.code == "TEST_CODE_000002"
            && item.current == "无"));
        assert!(violations
            .iter()
            .any(|item| item.rule == "板块集中度上限" && item.code == "测试板块"));
    }

    #[test]
    fn test_zero_cost_skips_stop_loss_and_alert_formatting_is_complete() {
        let positions = vec![pos("TEST_CODE_000001", 0.0, 1000)];
        let prices = std::collections::HashMap::from([("TEST_CODE_000001".to_string(), 10.0)]);
        let violations = check_position_limits(&positions, &prices, &HardLimits::default());
        assert!(violations.iter().all(|item| item.rule != "止损线"));
        assert_eq!(format_limit_alert(&[]), "");
        let alert = format_limit_alert(&violations);
        assert!(alert.starts_with("🚨 风控超标"));
        assert!(alert.contains("TEST_CODE_000001"));
    }
}
