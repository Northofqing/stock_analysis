...
fn main() -> Result<()> {
    ...
    let macro_news_context;
    {
        ...
        if !extra_codes.is_empty() {
            ...
            info!("📈 宏观AI推荐 {} 只，新增追加 {} 只（去重后）", extra_codes.len(), added);
        }
    }
    ...
    info!("待分析股票（共 {} 只）: {:?}", stock_codes.len(), stock_codes);
    ...
    if args.schedule || schedule_enabled {
        info!("定时任务模式已启用（来源: {}）", 
            if args.schedule { "命令行参数" } else { "环境变量" });
    ...
    if args.lhb_mode || lhb_mode_enabled {
        info!("模式: 龙虎榜选股分析（来源: {}）",
            if args.lhb_mode { "命令行参数" } else { "环境变量" });
...
fn run_scheduled_analysis(stock_codes: &[String], args: &Args) -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        if let Some(interval_minutes) = args.interval {
            // 模式1: 间隔执行
            run_interval_schedule(stock_codes, &config, interval_minutes, args.run_now, market_review_enabled).await
        } else if let Some(ref schedule_time) = args.schedule_time {
            // 模式2: 指定时间点执行（命令行参数优先）
            run_time_schedule(stock_codes, &config, schedule_time, args.weekdays.as_deref(), args.run_now, market_review_enabled).await
        } else {
            // 模式3: 使用环境变量配置
            let env_time = std::env::var("SCHEDULE_TIME").unwrap_or_else(|_| "09:30".to_string());
            info!("使用环境变量定时配置: SCHEDULE_TIME={}", env_time);
            run_time_schedule(stock_codes, &config, &env_time, args.weekdays.as_deref(), args.run_now, market_review_enabled).await
        }
    })
}
...
async fn run_time_schedule(
    ...
    let weekdays_str = if let Some(days) = weekdays {
        ...
        let names: Vec<String> = days.iter()
            .filter_map(|&d| if (1..=7).contains(&d) { 
    ...
    info!("定时模式: 每日 {:?} 执行{}", 
        time_points.iter().map(|(h, m)| format!("{}:{:02}", h, m)).collect::<Vec<_>>(),
        weekdays_str
    );
    ...
    loop {
        ...
        if let Some(next_time) = next_run {
            ...
            info!("\n\n开始定时分析 [{}]...", Local::now().format("%Y-%m-%d %H:%M:%S"));
            execute_analysis_with_mode(stock_codes, config, market_review_enabled).await;
            info!("\n");
        } else {
...
/// 执行分析（根据模式选择）
async fn execute_analysis_with_mode(stock_codes: &[String], config: &PipelineConfig, market_review_only: bool) {
...
async fn fetch_macro_recommended_codes() -> (Vec<String>, String) {
    ...
    let analyzer_clone = {
        ...
        if guard.is_available() { Some(guard.clone()) } else { None }
    };
    ...
    match tokio::time::timeout(
        ...
        Ok(Ok(rec_text)) => {
            ...
            let code_line_text = rec_text.lines()
...
fn run_lhb_analysis(args: &Args) -> Result<()> {
    ...
    let (good_stocks, stock_codes) = runtime.block_on(async {
        ...
        // 从环境变量或命令行参数获取配置
        let lhb_date = args.lhb_date.clone()
            .or_else(|| std::env::var("LHB_DATE").ok().filter(|s| !s.trim().is_empty()));
        ...
        let stock_codes: Vec<String> = good_stocks.iter()
