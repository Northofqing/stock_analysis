...
pub struct SaveUserPositionSnapshotReceipt { pub snapshot_row_id: i64, pub inserted: bool }

#[derive(Debug, Clone, PartialEq)]
pub struct UserPositionSnapshot {
    pub snapshot_row_id: i64, pub snapshot_id: String,
    pub effective_at: DateTime<FixedOffset>, pub confirmed_at: DateTime<FixedOffset>,
    pub source: String, pub confirm_empty: bool, pub evidence_sha256: String,
...
pub fn save_user_position_snapshot(input: &UserPositionSnapshotInput) -> Result<SaveUserPositionSnapshotReceipt, String> {
...
pub fn latest_user_position_snapshot() -> Result<Option<UserPositionSnapshot>, String> {
    let db = crate::database::DatabaseManager::get(); let mut conn = db.get_conn().map_err(|e| e.to_string())?;
    ...
    let Some((id,sid,ea,ca,source,empty,hash))=row else{return Ok(None)};
    let effective_at=DateTime::parse_from_rfc3339(&ea).map_err(|e|e.to_string())?; let confirmed_at=DateTime::parse_from_rfc3339(&ca).map_err(|e|e.to_string())?;
    ...
    Ok(Some(UserPositionSnapshot{snapshot_row_id:id,snapshot_id:sid,effective_at,confirmed_at,source,confirm_empty:empty!=0,evidence_sha256:hash,items:items.into_iter().map(|(code,name,q,cost_price)|UserPositionItemInput{code,name,quantity:q as u64,cost_price}).collect()}))
}
