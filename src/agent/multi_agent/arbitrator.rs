...
核心方法论（必须落到文字里，而非空喊）：
- **期望值(EV)思维**：EV = 胜率×平均盈利 − (1−胜率)×平均亏损；只在 EV 为正且赔率合理时给出买入。
...
4. 三情景概率必须求和=100%；EV 结论须与情景树自洽。
5. 章节固定（顺序不可改）：
   ## 一句话结论
   （一句话：方向 + 核心逻辑 + EV 正负，例：偏多，动量+资金共振，胜率约 55%、赔率 2.1，EV 为正）
   ## 因子归因
   - 价值：±N → 说明
   - 动量：±N → 说明
   - 质量：±N → 说明
   ...
   - 时间窗口：N/100
   - **加权综合：N/100**
6. 全文不超过 1100 字，要点精炼，禁止套话与"投资有风险"模板话术。"#;
