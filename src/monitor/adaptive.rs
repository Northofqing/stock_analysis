...
impl RuleTracker {
    pub fn new(name: &str, mode: RuleMode) -> Self {
        RuleTracker {
            name: name.into(), mode,
            shadow_signals: 0, shadow_hits: 0,
            active_signals: 0, active_hits: 0,
            promoted_at: None,
        }
    }
    ...
    pub fn shadow_hit_rate(&self) -> f64 {
        if self.shadow_signals == 0 { return 0.0; }
...
impl AdaptiveWeightManager {
    pub fn new(min_samples: u64, max_weekly_change: f64, shrinkage: f64) -> Self {
        ...
        AdaptiveWeightManager {
            rules: HashMap::new(), signal_weights,
            min_samples, max_weekly_change, shrinkage,
            locked: Mutex::new(()),
        }
    }
    ...
    pub fn register_rule(&mut self, name: &str) {
        self.rules.insert(name.into(), RuleTracker::new(name, RuleMode::Shadow));
    ...
    pub fn check_promotions(&mut self) -> Vec<String> {
        ...
        for (name, rule) in self.rules.iter_mut() {
            if rule.ready_for_promotion(self.min_samples, 0.55) {
                ...
                info!("[Adaptive] 规则 '{}' Shadow→Active（命中率 {:.0}%）",
                    name, rule.shadow_hit_rate() * 100.0);
            }
        }
        promoted
    }
    ...
    pub fn adjust_weights(&mut self) {
        ...
        for (source, w) in new_weights.iter_mut() {
            ...
            if hit_rate > 0.0 {
                let delta = if hit_rate > 0.6 { self.max_weekly_change }
                    else if hit_rate < 0.4 { -self.max_weekly_change }
                    else { 0.0 };
        ...
        for v in new_weights.values_mut() { *v /= total; }
        self.signal_weights = new_weights;
    }

    /// 计算某信号源的近期命中率（简化：统计所有规则中该源的命中率）
    fn source_hit_rate(&self, _source: SignalSource) -> f64 {
        let total: u64 = self.rules.values().map(|r| r.active_signals + r.shadow_signals).sum();
        let hits: u64 = self.rules.values().map(|r| r.active_hits + r.shadow_hits).sum();
        if total == 0 { 0.0 } else { hits as f64 / total as f64 }
    }

    /// 所有权重摘要
    pub fn weight_summary(&self) -> String {
        self.signal_weights.iter()
    ...
    pub fn shadow_summary(&self) -> String {
        let shadows: Vec<_> = self.rules.values()
            .filter(|r| matches!(r.mode, RuleMode::Shadow))
            .map(|r| format!("{}: {}/{} ({:.0}%)", r.name, r.shadow_hits, r.shadow_signals, r.shadow_hit_rate() * 100.0))
            .collect();
        if shadows.is_empty() { "无 Shadow 规则".into() } else { shadows.join("\n") }
    }
}
