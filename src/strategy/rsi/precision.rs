...
impl PrecisionRsiBacktest {
    ...
    pub fn run_single(
        ...
        for i in 0..n {
            ...
            let ma200 = match ma200_col.get(i) {
                Some(v) => v,
                None => {
                    signals.push(Signal::Hold);
                    continue;
            ...
            if shares < 1.0 {
                ...
                if cond1 && cond2 && cond3 && cond4 {
                    ...
                    let invest =
                        cash.min(self.config.initial_capital * self.config.max_position_pct);
    ...
    pub fn run_portfolio(
        ...
        let (portfolio_daily_values, portfolio_trades) = {
            ...
            let maps: Vec<HashMap<String, f64>> = all_single
                .iter()
                .map(|r| {
                    r.daily_values
                        .iter()
            ...
            let mut pv: Vec<(chrono::DateTime<Local>, f64)> = date_set
                .iter()
                .map(|ds| {
            ...
            if let Some(&(_, first_val)) = pv.first() {
                if first_val > 0.0 {
                    ...
                    for (_, v) in pv.iter_mut() {
                        *v *= scale;
                    }
                }
            }
            let mut all_trades: Vec<Trade> =
                all_single.iter().flat_map(|r| r.trades.clone()).collect();
...
impl PrecisionRsiStrategy {
    pub fn new(config: PrecisionRsiConfig) -> Self {
        Self {
            backtest: PrecisionRsiBacktest::new(config),
        }
    }
}
...
impl Default for PrecisionRsiStrategy {
    fn default() -> Self {
        Self::new(PrecisionRsiConfig::default())
    }
}
