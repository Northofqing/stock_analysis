...
const ACTIVE_PRESENTATIONS: [PresentationTuple; 59] = [
    (
        "T-01-account-mode",
    ...
    (
        // 2026-08-07 BR-192 收尾: 盘中指标告警 counted 接线 (与 registry 同步)。
    ...
    (
        // G5b 深链归因 (2026-08-22) — 与 presentation_registry.rs descriptor 同构
    ...
    (
        // NewsAI 分析卡 (2026-09-22) — 全 7 触点新增。该卡此前经 BR-172
        // 专用状态机物理直推、无生产呈现 tuple; 接入 counted 准入层必须
        // 补注册 (counted 门要求 ProductionPresentationToken)。
...
const ALL_PUSH_KINDS: [PushKind; 66] = [
    PushKind::PaperSell,
    PushKind::SnapshotStale,
    PushKind::HoldingEvent,
    PushKind::DailyReport,
...
fn validate_manifest(
    ...
    if families.len() != 75 {
        ...
        // 2026-09-22 NewsAI 分析卡 (U-04-news-ai-analysis): 75 = ACTIVE 59 +
        // DISABLED 13 + RETIRED 3。
    ...
    // 2026-09-22: NewsAI 分析卡 (U-04-news-ai-analysis) 全 7 触点 → Active
    // 家族 +1 (两个分支各 +1), family total 74 → 75; PushKind inventory
    // 65 → 66。
...
fn validate_descriptor_bijection(
    ...
    if descriptors.len() != 61 {
        ...
        // + 2026-09-22 NewsAI 分析卡 (U-04-news-ai-analysis) = 61。
        return Err("BR-196 production descriptor count must be 61".to_string());
    }
...
fn project_push_kind_lifecycle(families: &[TemplateFamily]) -> Result<LifecycleCounts, String> {
    ...
    if all.len() != 66 || projected.len() != 66 || projected.keys().any(|kind| !all.contains(kind))
    {
        // 交付物 A (2026-08-20) 新增 AttributionDaily PushKind 后 ALL_PUSH_KINDS 61 → 62;
        // G5b (2026-08-22) +1 → 63
        // 2026-09-22 NewsAI 分析卡 (NewsAiAnalysis) +3 → 66
        return Err("BR-196 PushKind inventory is not an exact 66-kind cover".to_string());
    }
...
mod tests {
    ...
    fn br196_default_and_active_newsflash_matrices_are_exact() {
        ...
        // 2026-09-22: NewsAI 分析卡 (U-04-news-ai-analysis) → 75 / 66。
        assert_eq!(
