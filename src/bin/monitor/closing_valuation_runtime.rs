...
pub async fn run_closing_valuation_once(date: NaiveDate) -> Result<database::closing_valuation::SaveClosingValuationReceipt, String> {
...
fn run_blocking(date: NaiveDate) -> Result<database::closing_valuation::SaveClosingValuationReceipt, String> {
    ...
    for item in &snapshot.items {
        let bars = provider.get_daily_data(&item.code, 10).map_err(|e| format!("{} RustDX: {e}", item.code))?;
        let closes: Vec<(NaiveDate, f64)> = bars
            .iter()
            .filter(|b| b.adjust == AdjustType::None && b.settled && b.close.is_finite() && b.close > 0.0)
            .map(|b| (b.date, b.close))
            .collect();
        let current = closes.iter().find(|(d, _)| *d == date)
            .ok_or_else(|| format!("BR-147 {} missing settled RustDX close for {date}", item.code))?;
        prices.push(ClosingPriceEvidence {
            code: item.code.clone(), price_date: date, close: current.1,
            provider: "rustdx_none".into(), evidence_hash: format!("{}:{}:{:.6}", item.code, date, current.1),
        });
        if let Some((_, prev)) = closes.iter().find(|(d, _)| *d < date) {
            previous.push((item.code.clone(), *prev));
        }
    }
    let view = calculate_closing_valuation(&snapshot.items, &prices, &previous, date, "rustdx_none")?;
    database::closing_valuation::save_closing_valuation(&view).map_err(|e| format!("BR-147 persist failed: {e}"))
}
...
mod tests {
    ...
    fn eligibility_starts_at_close() {
        let before = Local::now().date_naive().and_hms_opt(14, 59, 59).unwrap().and_local_timezone(*Local::now().offset()).single().unwrap();
        assert!(!eligible_after_close(before));
        let after = Local::now().date_naive().and_hms_opt(15, 0, 0).unwrap().and_local_timezone(*Local::now().offset()).single().unwrap();
