...
fn collect_model_configs() -> Vec<ActiveModelConfig> {
    ...
    if let Some(key) = env::var("DOUBAO_API_KEY").ok().filter(|k| !k.is_empty()) {
        let base = env::var("DOUBAO_BASE_URL")
            .unwrap_or_else(|_| "https://ark.cn-beijing.volces.com/api/v3".to_string());
        let model =
            env::var("DOUBAO_MODEL").unwrap_or_else(|_| "doubao-seed-2-0-pro-260215".to_string());
    ...
    if let Some(key) = env::var("GEMINI_API_KEY").ok().filter(|k| !k.is_empty()) {
        let base = env::var("GEMINI_BASE_URL").unwrap_or_else(|_| {
...
请根据个股数据和行业板块联动，给出最终深入的评估。".to_string();
    ...
    let stock_codes: Vec<&str> = stock_list_env
        .split(',')
        .map(|s| s.trim())
        .filter(|s| !s.is_empty())
        .collect();
    ...
    for code in stock_codes {
        ...
        println!(
            ">>> 从 SQLite 数据库读取 Agent 内部思考日志 (Scratchpad) for Session {}:",
            agent.session_id
        );
