...
fn test_e2e_full_pipeline_smoke() {
    ...
    // 修复 P0-1: 量化产品经理验收标准
    assert!(score.event_risk_score >= 60, "强事件高分: {}", score.event_risk_score);
    assert!(score.trade_signal_score.is_none(), "沙盘阶段 trade_signal=None");
...
fn test_e2e_with_winrate_qualifies_for_gray() {
    ...
    let metrics = StageMetrics {
        shadow_days: 70, winrate_samples: 200,
        winrate_pct: winrate_score, calmar_ratio: 1.2, gray_days: 0,
    };
...
fn test_e2e_pipeline_with_no_data() {
    ...
    let inputs = ScoreInputs {
        event_strength: 50, event_certainty: 50,
...
fn test_e2e_weight_version_propagates() {
    ...
    let inputs = ScoreInputs {
        event_strength: 70, event_certainty: 70,
