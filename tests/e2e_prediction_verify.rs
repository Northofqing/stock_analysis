...
async fn test_verify_predictions_writes_real_actual_change() {
    ...
    retry_db(|| {
        let _ =
            diesel::sql_query("DELETE FROM prediction_tracker WHERE stock_code = 'TEST_CODE_001'")
    ...
    let row = db
        .get_prediction_by_code_date("TEST_CODE_001", &yesterday)
...
async fn test_verify_predictions_miss_for_bearish_prediction_on_up_day() {
    ...
    retry_db(|| {
        let _ =
            diesel::sql_query("DELETE FROM prediction_tracker WHERE stock_code = 'TEST_CODE_002'")
                .execute(&mut *db.get_conn().unwrap());
    ...
    // 4. 断言: 看空+次日涨 → hit=0
    let row = db
        .get_prediction_by_code_date("TEST_CODE_002", &yesterday)
        .expect("应能查回");
    assert_eq!(
        row.hit,
        Some(0),
        "看空+次日涨 → hit=0 (未命中), row={:?}",
        row
    );
